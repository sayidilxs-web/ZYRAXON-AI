...
export function glossaryFile(locale: Locale) {
  if (locale === "zh") return ".zyraxon/glossary/zh-cn.md"
  if (locale === "zht") return ".zyraxon/glossary/zh-tw.md"
  return `.zyraxon/glossary/${locale}.md`
}
...
async function translate(
  ...
  env.ZYRAXON_DISABLE_PROJECT_CONFIG = "1"
  env.ZYRAXON_CONFIG_CONTENT = JSON.stringify(
    translationConfig(
      agent,
      model,
...
function isolatedEnvironment() {
  ...
  delete env.ZYRAXON_CONFIG
  delete env.ZYRAXON_CONFIG_DIR
  delete env.ZYRAXON_CONFIG_CONTENT
  delete env.ZYRAXON_PERMISSION
  delete env.ZYRAXON_AUTO_SHARE
  return env
}
