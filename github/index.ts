...
try {
  ...
  console.log("zyraxon session", session.id)
  if (shareId) {
...
function useShareUrl() {
  return isMock() ? "https://dev.zyraxonai.lovable.app" : "https://zyraxonai.lovable.app"
}
...
async function getAccessToken() {
  ...
  let response
  if (isMock()) {
    response = await fetch("https://api.zyraxon.ai/exchange_github_app_token_with_pat", {
      method: "POST",
      headers: {
    ...
    response = await fetch("https://api.zyraxon.ai/exchange_github_app_token", {
      method: "POST",
      headers: {
