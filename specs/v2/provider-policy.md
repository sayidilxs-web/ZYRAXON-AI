...
Both `action` and `resource` use ZYRAXON's existing wildcard matching behavior.
...
The relative policy precedence of direct project files and `.ZYRAXON` files is intentionally deferred until `.ZYRAXON` configuration is reviewed.
...
Plugins must not be allowed to add, remove, or override policy statements. Plugins can contribute functionality or configured providers; policy determines whether ZYRAXON permits an operation through its managed execution paths.
