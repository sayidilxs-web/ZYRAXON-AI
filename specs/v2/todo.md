...
The ZYRAXON server has moved to the Effect HttpApi backend. Remaining work is
...
- remove the public in-memory `@ZYRAXON-ai/llm` tool loop after replacing its
...
- plugins get global "ZYRAXON" instance like in that post i showed
- ZYRAXON instance has stuff like `ZYRAXON.session.prompt()` or
  `ZYRAXON.tool.register({...})`
...
- decide whether to preserve deprecated `@ZYRAXON-ai/llm` orchestration exports
