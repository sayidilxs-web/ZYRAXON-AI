...
Move the canonical ZYRAXON terminal application from
`packages/ZYRAXON/src/cli/cmd/tui` into a self-contained workspace package while
...
generic presentation libraries. It must not depend on `packages/ZYRAXON`,
`packages/cli`, or `@ZYRAXON-ai/core`.

The SDK is the TUI's ZYRAXON boundary. Missing backend data or operations must
...
- Do not preserve private imports by creating aliases from `packages/tui` back
  into `packages/ZYRAXON`.
- Do not replace private `packages/ZYRAXON` imports with `@ZYRAXON-ai/core`
...
- Keep legacy CLI command parsing, server startup, worker management,
  authentication, and config discovery outside `@ZYRAXON-ai/tui`.
...
- ZYRAXON domain data displayed by the TUI
...
```text
packages/ZYRAXON/src/cli/cmd/tui
```

Its private dependency on `packages/ZYRAXON` is primarily expressed through
the `@/*` TypeScript alias, which resolves to `packages/ZYRAXON/src/*`.
...
Status: Completed. The private `@ZYRAXON-ai/tui` workspace package now has an
...
- Add `packages/tui/package.json` with the name `@ZYRAXON-ai/tui`.
...
- The package has no dependency on `ZYRAXON`, `@ZYRAXON-ai/cli`, or
  `@ZYRAXON-ai/core`.
...
primitives now live in `@ZYRAXON-ai/tui`. The legacy host consumes explicit
...
and build inputs through `@ZYRAXON-ai/tui/runtime`. Movable app, component,
route, and feature-plugin code no longer reads ZYRAXON globals or process state;
...
Replace process-global ZYRAXON state with resolved TUI inputs.
...
`@ZYRAXON-ai/tui/config` now owns schemas, defaults, keybind resolution, the
...
- Keep config path discovery, project/global precedence, migration, variable
  expansion, and plugin package installation in `packages/ZYRAXON` initially.
...
canonical in `@ZYRAXON-ai/tui`. Configured references resolve through the new
...
adapters rather than ZYRAXON domain implementations.

Make the SDK the only ZYRAXON domain boundary used by the TUI.
...
- Domain-facing TUI code imports ZYRAXON data and operations only from
  `@ZYRAXON-ai/sdk`.
...
`@ZYRAXON-ai/tui`. The legacy host injects a narrow plugin host that retains
...
- Move plugin rendering tests to `packages/tui`; retain installation/loading
  integration tests in `packages/ZYRAXON`.
...
- The package has no imports from `packages/ZYRAXON`, `packages/cli`, or
  `@ZYRAXON-ai/core`.
...
public `@ZYRAXON-ai/tui` root while retaining worker/server/config/plugin and
...
- Keep the legacy yargs commands corresponding to current `thread.ts` and
  `attach.ts` in `packages/ZYRAXON`.
- Keep the legacy embedded worker and server startup in `packages/ZYRAXON`.
- Change those adapters to load config, create transport inputs, and call the
  public `@ZYRAXON-ai/tui` API.
...
- Remove obsolete `@tui/*` path mappings from `packages/ZYRAXON`.
- Remove stale test fixtures and update all imports to package exports.
- Narrow `@ZYRAXON-ai/tui` exports to intentional public entrypoints.
...
- No source under `packages/tui` imports `@/...`, `@ZYRAXON-ai/core`, or either
  executable package.
...
- TUI package imports do not reach into executable or backend implementation
  packages.
- SDK wire data is treated as the source of truth for ZYRAXON domain state.
...
cd packages/ZYRAXON && bun typecheck
...
rg '@ZYRAXON-ai/core|packages/ZYRAXON|packages/cli' packages/tui
rg 'src/cli/cmd/tui|@tui/' packages/ZYRAXON/src packages/ZYRAXON/test
```
...
- Build the current-platform `packages/ZYRAXON` binary.
