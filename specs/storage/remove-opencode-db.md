# Remove `packages/ZYRAXON/src/storage/db.ts`

## Goal

Remove all production usages of the legacy `packages/ZYRAXON/src/storage/db.ts` module.
...
This does not mean removing SQLite or Drizzle everywhere in one step. The smaller target is deleting the ZYRAXON legacy wrapper by moving call sites onto deeper modules or onto the core/effect database adapter directly.

## Current Inventory

Production imports from `packages/ZYRAXON/src/storage/db.ts` are concentrated in 22 source files:

- `packages/ZYRAXON/src/account/repo.ts`
- `packages/ZYRAXON/src/cli/cmd/db.ts`
- `packages/ZYRAXON/src/cli/cmd/import.ts`
- `packages/ZYRAXON/src/cli/cmd/stats.ts`
- `packages/ZYRAXON/src/control-plane/workspace.ts`
- `packages/ZYRAXON/src/index.ts`
- `packages/ZYRAXON/src/node.ts`
- `packages/ZYRAXON/src/permission/index.ts`
- `packages/ZYRAXON/src/project/project.ts`
- `packages/ZYRAXON/src/server/projectors.ts`
- `packages/ZYRAXON/src/server/routes/instance/httpapi/handlers/sync.ts`
- `packages/ZYRAXON/src/server/shared/fence.ts`
- `packages/ZYRAXON/src/session/message-v2.ts`
- `packages/ZYRAXON/src/session/projectors.ts`
- `packages/ZYRAXON/src/session/prompt.ts`
- `packages/ZYRAXON/src/session/session.ts`
- `packages/ZYRAXON/src/session/todo.ts`
- `packages/ZYRAXON/src/share/share-next.ts`
- `packages/ZYRAXON/src/storage/db.ts`
- `packages/ZYRAXON/src/sync/index.ts`
- `packages/ZYRAXON/src/worktree/index.ts`
...
Status: Completed. Startup, the public node export, and database CLI tooling no longer import the legacy ZYRAXON database wrapper; `packages/ZYRAXON/src/storage/db.ts` has been deleted.
...
- Move database path and client startup behind the core/effect database module rather than the ZYRAXON wrapper.
...
Status: Completed. `SyncEvent` and the ZYRAXON projector boundary were removed; session/message event projection now lives in core EventV2/projector infrastructure.
...
Status: Completed. These services no longer import the legacy ZYRAXON database wrapper.
...
Status: Completed. Session/message reads and projector writes have moved off the legacy ZYRAXON database wrapper.
...
Status: Completed. Remaining one-off CLI/admin reads and writes now use core database services or domain services instead of the legacy ZYRAXON database wrapper.
...
All migration groups are complete or superseded. `packages/ZYRAXON/src/storage/db.ts` has been deleted.

## Superseded: Data Migrations

Status: Superseded. No ZYRAXON data-migration group remains.

The previous ZYRAXON `data-migration.ts` service only backfilled session usage from message rows. That work is now covered by core database migration `packages/core/src/database/migration/20260510033149_session_usage.ts`, so there is no separate ZYRAXON data-migration group.
...
- Existing schema ownership remains in `packages/core/src/**/*.sql.ts`; do not move table definitions back into `packages/ZYRAXON`.

## Verification Commands

- `rg "@/storage/db|./storage/db|Database\.(use|transaction|effect|Client|getPath)|\bTxOrDb\b|\bTransaction\b" packages/ZYRAXON/src`
- `bun typecheck` from `packages/ZYRAXON`
- Relevant package tests from `packages/ZYRAXON`, not the repo root
