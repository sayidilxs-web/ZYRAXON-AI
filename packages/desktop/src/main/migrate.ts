...
// `zyraxon.settings.dat` is special: it maps to the `zyraxon.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"zyraxon.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // zyraxon.settings.dat → the electron settings store ("zyraxon.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "zyraxon.global.dat").
