...
function clearHighlightFind() {
  ...
  if (!api) return
  api.delete("zyraxon-find")
  api.delete("zyraxon-find-current")
}
...
export function createFileFind(opts: CreateFileFindOptions) {
  ...
  const setHighlights = (ranges: Range[], currentIndex: number) => {
    ...
    if (active) api.set("zyraxon-find-current", new Highlight(active))

    const rest = ranges.filter((_, i) => i !== currentIndex)
    if (rest.length > 0) api.set("zyraxon-find", new Highlight(...rest))
    return true
  }
