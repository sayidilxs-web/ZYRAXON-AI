...
export async function startChromeTrace(page: Page, name: string) {
  const directory = process.env.ZYRAXON_PERFORMANCE_TRACE_DIR
  if (!directory) return

  const selectors = process.env.ZYRAXON_PERFORMANCE_SELECTOR_TRACE === "1"
...
export async function prepareChromeTrace(
  ...
  const run = process.env.ZYRAXON_PERFORMANCE_RUN_ID ?? "manual"
