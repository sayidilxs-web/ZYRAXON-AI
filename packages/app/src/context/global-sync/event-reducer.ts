...
export function applyDirectoryEvent(input: {
  ...
  switch (event.type) {
    ...
    case "session.created": {
      ...
      if (info.parentID) {
        window.dispatchEvent(new CustomEvent("zyraxon:subagent-event", {
          detail: { type: "created", sessionID: info.id, parentID: info.parentID, agent: info.agent, title: info.title },
        }))
      }
      break
    }
    ...
    case "session.status": {
      ...
      window.dispatchEvent(new CustomEvent("zyraxon:subagent-event", {
        detail: { type: "status", sessionID: props.sessionID, status: props.status },
      }))
      break
    }
    case "message.updated": {
      ...
      window.dispatchEvent(new CustomEvent("zyraxon:subagent-event", {
        detail: { type: "message-mapping", messageID: info.id, sessionID: info.sessionID },
      }))
      break
    }
    ...
    case "message.part.updated": {
      ...
      if (part.type === "tool") {
        ...
        const stateType = toolPart.state?.status ?? "running"
        window.dispatchEvent(new CustomEvent("zyraxon:subagent-event", {
          detail: {
            type: "part-updated",
            partID: part.id,
