...
declare const ZYRAXON_LIBC: string | undefined
...
const watcher = lazy((): typeof import("@parcel/watcher") | undefined => {
  try {
    const libc = typeof ZYRAXON_LIBC === "undefined" ? undefined : ZYRAXON_LIBC
...
export class Service extends Context.Service<Service, Interface>()("@zyraxon/v2/FileWatcher") {}

const layer = Layer.effect(
  Service,
  Effect.gen(function* () {
    if (yield* Flag.ZYRAXON_EXPERIMENTAL_DISABLE_FILEWATCHER) return Service.of({})
    ...
    if (location.vcs && (yield* Flag.ZYRAXON_EXPERIMENTAL_FILEWATCHER)) {
