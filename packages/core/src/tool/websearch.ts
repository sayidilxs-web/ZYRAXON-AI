...
export class ConfigService extends Context.Service<ConfigService, Config>()("@zyraxon/v2/WebSearchConfig") {}

/** Isolates the retained product environment contract from the generic tool implementation. */
export const defaultConfigLayer = Layer.sync(ConfigService, () =>
  ConfigService.of({
    provider:
      process.env.ZYRAXON_WEBSEARCH_PROVIDER === "exa" || process.env.ZYRAXON_WEBSEARCH_PROVIDER === "parallel"
        ? process.env.ZYRAXON_WEBSEARCH_PROVIDER
        : undefined,
    enableExa: truthy("ZYRAXON_EXPERIMENTAL") || truthy("ZYRAXON_ENABLE_EXA") || truthy("ZYRAXON_EXPERIMENTAL_EXA"),
    enableParallel: truthy("ZYRAXON_ENABLE_PARALLEL") || truthy("ZYRAXON_EXPERIMENTAL_PARALLEL"),
