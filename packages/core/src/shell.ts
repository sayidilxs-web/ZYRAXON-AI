...
async function unix() {
  ...
  return ["/bin/bash", "/bin/zsh", "/bin/sh", "/usr/bin/fish", "/usr/bin/nu", "/usr/bin/elvish"]
}
...
export function gitbash() {
  ...
  if (Flag.ZYRAXON_GIT_BASH_PATH) return Flag.ZYRAXON_GIT_BASH_PATH
  const git = which("git")
  if (!git) return
...
export function args(file: string, command: string, cwd: string) {
  const n = name(file)
  if (n === "nu" || n === "fish" || n === "elvish" || n === "xonsh") return ["-c", command]
  if (n === "oil") return ["-c", command]
  if (n === "zsh") {
    return [
      "-l",
      "-c",
