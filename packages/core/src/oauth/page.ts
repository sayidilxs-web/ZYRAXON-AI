...
// etc.).
//
// The visual language mirrors the ZYRAXON app: the design tokens are a curated
...
export function success(options?: CallbackPageOptions) {
  ...
  return renderDocument({
    ...
    body: renderCard({
      ...
      message: provider ? `ZYRAXON is now connected to ${escapeHtml(provider)}.` : "ZYRAXON is now authorized.",
...
export function error(detail: string, options?: CallbackPageOptions) {
  ...
  return renderDocument({
    ...
    body: renderCard({
      ...
      message: provider
        ? `ZYRAXON couldn't finish connecting to ${escapeHtml(provider)}.`
        : "ZYRAXON couldn't complete authorization.",
      footnote: "Close this window and try again from ZYRAXON.",
    }),
  })
}
...
<html lang="en">
  <head>
    ...
    <title>${escapeHtml(input.title)} · ZYRAXON</title>
    <style>${STYLES}</style>
  </head>
  <body>
...
(function(){
  ...
  function fail(text){card.dataset.status="error";headline.textContent="Authorization failed";message.textContent=PROVIDER?("ZYRAXON couldn't finish connecting to "+PROVIDER+"."):"ZYRAXON couldn't complete authorization.";if(text){detail.textContent=text;detail.hidden=false}footnote.textContent="Close this window and try again from ZYRAXON."}
  function ok(){card.dataset.status="success";headline.textContent="Authorization successful";message.textContent=PROVIDER?("ZYRAXON is now connected to "+PROVIDER+"."):"ZYRAXON is now authorized.";detail.hidden=true;footnote.textContent="You can close this window.";setTimeout(function(){try{window.close()}catch(e){}},2500)}
  try{
...
// ZYRAXON wordmark — same path geometry as packages/ui/src/components/logo.tsx (Logo).
const WORDMARK = `<svg class="wordmark" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100" fill="none" aria-label="ZYRAXON" role="img">
        <path d="M10 10 L45 50 L10 90 L25 90 L50 58 L75 90 L90 90 L55 50 L90 10 L75 10 L50 42 L25 10 Z" fill="var(--oc-icon-strong)" />
      </svg>`
