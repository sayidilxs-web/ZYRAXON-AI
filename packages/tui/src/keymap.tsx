...
export function createOpencodeModeStack(keymap: OpenTuiKeymap) {
  ...
  const offFields = keymap.registerLayerFields({
    mode(value, ctx) {
      ctx.require(ZYRAXON_MODE_KEY, value)
    },
  })
  ...
  const update = () => {
    keymap.setData(ZYRAXON_MODE_KEY, stack.at(-1)?.mode ?? ZYRAXON_BASE_MODE)
  }

  const stackApi = {
    current() {
      return stack.at(-1)?.mode ?? ZYRAXON_BASE_MODE
    },
    push(mode: string) {
      if (disposed) return () => {}
    ...
    dispose() {
      ...
      stack.length = 0
      offFields()
      keymap.setData(ZYRAXON_MODE_KEY, undefined)
