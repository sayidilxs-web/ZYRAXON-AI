...
const CUSTOM_PROVIDER_OPTION_VALUE = "__ZYRAXON_custom_provider__"
    ...
    const value = await DialogPrompt.show(dialog, "Other", {
      ...
      description: () => (
        <text fg={theme.textMuted}>
          This only stores a credential. Configure the provider in zyraxon.json to use it.
        </text>
      ),
    })
    ...
    <DialogPrompt
      ...
      description={() =>
        ({
          opencode: (
            <box gap={1}>
              <text fg={theme.textMuted}>
                ZYRAXON Zen gives you access to all the best coding models at the cheapest prices with a single API
                key.
              </text>
              <text fg={theme.text}>
          ...
          "opencode-go": (
            <box gap={1}>
              <text fg={theme.textMuted}>
                ZYRAXON Go is a $10 per month subscription that provides reliable access to popular open coding models
      ...
      onConfirm={async (value) => {
        ...
        if (props.custom && !sync.data.provider_next.all.some((provider) => provider.id === props.providerID)) {
          toast.show({
            variant: "info",
            message: `Saved credential for ${props.providerID}. Configure it in zyraxon.json to use it.`,
          })
          dialog.clear()
          return
        }
