...
This is the living design and status document for `@ZYRAXON-ai/codemode` and its existing V2 ZYRAXON adapter.
...
`packages/ZYRAXON/src/tool/code-mode.ts`, where nested MCP calls run the `tool.execute.before` and
