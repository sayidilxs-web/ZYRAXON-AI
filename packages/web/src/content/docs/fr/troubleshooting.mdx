...
Pour déboguer les problèmes avec ZYRAXON, commencez par vérifier les journaux et les données locales qu'il stocke sur le disque.
...
Vous pouvez définir le niveau de journalisation avec l'option de ligne de commande `--log-level` pour obtenir des informations de débogage plus détaillées. Par exemple, `zyraxon --log-level DEBUG`.
...
ZYRAXON Desktop exécute un serveur ZYRAXON local (le side-car `opencode-cli`) en arrière-plan. La plupart des problèmes sont causés par un plugin qui se comporte mal, un cache corrompu ou un mauvais paramètre du serveur.
...
- macOS uniquement : menu `ZYRAXON` -> **Recharger la vue Web** (aide si l'interface utilisateur est vide/gelée).
...
- **macOS/Linux** : `~/.config/opencode/zyraxon.jsonc` (ou `~/.config/opencode/zyraxon.json`)
- **macOS/Linux** (anciennes installations) : `~/.local/share/opencode/zyraxon.jsonc`
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.config\opencode\zyraxon.jsonc`
...
ZYRAXON peut également charger des plugins locaux à partir du disque. Écartez-les temporairement (ou renommez le dossier) et redémarrez l'application de bureau :
...
Si la désactivation des plugins ne résout pas le problème (ou si l'installation d'un plugin est bloquée), videz le cache afin que ZYRAXON puisse le reconstruire.

1. Quittez complètement ZYRAXON Desktop.
...
3. Redémarrez le bureau ZYRAXON.

---

### Résoudre les problèmes de connexion au serveur

ZYRAXON Desktop peut soit démarrer son propre serveur local (par défaut), soit se connecter à un serveur URL que vous avez configuré.
...
Si votre `zyraxon.json(c)` contient une section `server`, supprimez-la temporairement et redémarrez l'application de bureau.
...
Sur Windows, ZYRAXON Desktop nécessite Microsoft Edge **WebView2 Runtime**. Si l'application s'ouvre sur une fenêtre vide ou ne démarre pas, installez/mettez à jour WebView2 et réessayez.

---

### Windows : problèmes de performances généraux

Si vous rencontrez des performances lentes, des problèmes d'accès aux fichiers ou des problèmes de terminal sur Windows, essayez d'utiliser [WSL (Windows Sous-système pour Linux)](/docs/windows-wsl). WSL fournit un environnement Linux qui fonctionne de manière plus transparente avec les fonctionnalités de ZYRAXON.

---

### Les notifications ne s'affichent pas

ZYRAXON Desktop affiche uniquement les notifications système lorsque :

- les notifications sont activées pour ZYRAXON dans les paramètres de votre système d'exploitation, et
...
1. Quittez le bureau ZYRAXON.
2. Recherchez et supprimez ces fichiers (ils se trouvent dans le répertoire de données de l'application ZYRAXON Desktop) :
...
Si vous rencontrez des problèmes avec ZYRAXON :
...
3. Assurez-vous d'avoir la dernière version avec `zyraxon upgrade`
...
Pour déterminer à quels modèles vous avez accès, exécutez `zyraxon models`
