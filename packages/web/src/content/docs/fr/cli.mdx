---
title: CLI
description: Options et commandes de la CLI ZYRAXON.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

La CLI ZYRAXON démarre par défaut le [TUI](/docs/tui) lorsqu'il est exécuté sans aucun argument.

```bash
opencode
```

Mais elle accepte également les commandes documentées sur cette page. Cela vous permet d'interagir avec ZYRAXON par programme.
...
Démarrez l’interface utilisateur du terminal ZYRAXON.
...
La CLI ZYRAXON dispose également des commandes suivantes.

---

### agent

Gérer les agents pour ZYRAXON.
...
Attachez un terminal à un serveur backend ZYRAXON déjà en cours d'exécution démarré via les commandes `serve` ou `web`.

```bash
opencode attach [url]
```

Cela permet d'utiliser le TUI avec un backend ZYRAXON distant. Par exemple :
...
ZYRAXON est alimenté par la liste des fournisseurs sur [Models.dev](https://models.dev), vous pouvez donc utiliser `zyraxon auth login` pour configurer les clés API pour tout fournisseur que vous souhaitez utiliser. Ceci est stocké dans `~/.local/share/opencode/auth.json`.

```bash
opencode auth login
```

Lorsque ZYRAXON démarre, il charge les fournisseurs à partir du fichier d'informations d'identification, ainsi que les clés définies dans vos environnements ou un fichier `.env` dans votre projet.
...
Utilisez l'option `--refresh` pour mettre à jour la liste des modèles mis en cache. Ceci est utile lorsque de nouveaux modèles ont été ajoutés à un fournisseur et que vous souhaitez les voir dans ZYRAXON.
...
Vous pouvez également vous connecter à une instance `zyraxon serve` en cours d'exécution pour éviter les temps de démarrage à froid du serveur MCP à chaque exécution :
...
Démarrez un serveur ZYRAXON sans interface graphique pour un accès API. Consultez le [serveur docs](/docs/server) pour l'interface complète HTTP.

```bash
opencode serve
```

Cela démarre un serveur HTTP qui fournit un accès API aux fonctionnalités d'ZYRAXON sans l'interface TUI. Définissez `OPENCODE_SERVER_PASSWORD` pour activer l'authentification de base HTTP (le nom d'utilisateur par défaut est `opencode`).
...
Liste toutes les sessions ZYRAXON.
...
Affichez les statistiques d'utilisation et de coût des jetons pour vos sessions ZYRAXON.
...
Importez les données de session à partir d'un fichier JSON ou d'une URL de partage ZYRAXON.

```bash
opencode import <file>
```

Vous pouvez importer à partir d'un fichier local ou d'une URL de partage ZYRAXON.

```bash
opencode import session.json
opencode import https://zyraxonai.lovable.app/share/abc123
```
...
Démarrez un serveur ZYRAXON sans interface graphique avec une interface Web.

```bash
opencode web
```

Cela démarre un serveur HTTP et ouvre un navigateur Web pour accéder à ZYRAXON via une interface Web. Définissez `OPENCODE_SERVER_PASSWORD` pour activer l'authentification de base HTTP (le nom d'utilisateur par défaut est `opencode`).
...
Désinstallez ZYRAXON et supprimez tous les fichiers associés.
...
ZYRAXON peut être configuré à l'aide de variables d'environnement.
