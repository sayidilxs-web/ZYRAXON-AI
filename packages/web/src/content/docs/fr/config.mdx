---
title: Configuration
description: Utilisation de la configuration JSON ZYRAXON.
---

Vous pouvez configurer ZYRAXON à l'aide d'un fichier de configuration JSON.

---

## Format

ZYRAXON prend en charge les formats **JSON** et **JSONC** (JSON avec commentaires).
...
2. **Configuration globale** (`~/.config/opencode/zyraxon.json`) - préférences utilisateur
3. **Configuration personnalisée** (`OPENCODE_CONFIG` env var) - remplacements personnalisés
4. **Configuration du projet** (`zyraxon.json` dans le projet) - paramètres spécifiques au projet
...
Placez votre configuration globale ZYRAXON dans `~/.config/opencode/zyraxon.json`. Utilisez la configuration globale pour les préférences de l'utilisateur telles que les fournisseurs, les modèles et les autorisations.
...
Ajoutez `zyraxon.json` à la racine de votre projet. La configuration du projet a la priorité la plus élevée parmi les fichiers de configuration standard : elle remplace les configurations globales et distantes.
...
Lorsque ZYRAXON démarre, il recherche un fichier de configuration dans le répertoire actuel ou remonte jusqu'au répertoire Git le plus proche.
...
Le fichier de configuration a un schéma défini dans [**`zyraxon.ai/config.json`**](https://zyraxon.ai/config.json).

La configuration TUI utilise [**`zyraxon.ai/tui.json`**](https://zyraxon.ai/tui.json).
...
Les anciennes clés `theme`, `keybinds` et `tui` dans `zyraxon.json` sont obsolètes et migrées automatiquement lorsque cela est possible.
...
Vous pouvez configurer les paramètres du serveur pour les commandes `zyraxon serve` et `zyraxon web` via l'option `server`.
...
- `mdns` - Activer la découverte du service mDNS. Cela permet à d'autres appareils du réseau de découvrir votre serveur ZYRAXON.
...
Vous pouvez configurer les fournisseurs et les modèles que vous souhaitez utiliser dans votre configuration ZYRAXON via les options `provider`, `model` et `small_model`.
...
L'option `small_model` configure un modèle distinct pour les tâches légères comme la génération de titres. Par défaut, ZYRAXON essaie d'utiliser un modèle moins cher s'il est disponible auprès de votre fournisseur, sinon il revient à votre modèle principal.
...
L'agent par défaut doit être un agent principal (et non un sous-agent). Il peut s'agir d'un agent intégré tel que `"build"` ou `"plan"`, ou d'un [agent personnalisé](/docs/agents) que vous avez défini. Si l'agent spécifié n'existe pas ou est un sous-agent, ZYRAXON reviendra à `"build"` avec un avertissement.

Ce paramètre s'applique à toutes les interfaces : TUI, CLI (`zyraxon run`), application de bureau et GitHub Action.
...
ZYRAXON téléchargera automatiquement toutes les nouvelles mises à jour au démarrage. Vous pouvez désactiver cela avec l'option `autoupdate`.
...
[Plugins](/docs/plugins) étendent ZYRAXON avec des outils, des hooks et des intégrations personnalisés.
...
Ceci est utile lorsque vous souhaitez restreindre ZYRAXON à l'utilisation de fournisseurs spécifiques plutôt que de les désactiver un par un.
