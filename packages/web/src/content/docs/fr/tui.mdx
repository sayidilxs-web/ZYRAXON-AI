---
title: TUI
description: Utilisation de l’interface utilisateur du terminal ZYRAXON.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

ZYRAXON fournit une interface de terminal interactif ou TUI pour travailler sur vos projets avec un LLM.

L’exécution de ZYRAXON démarre le TUI pour le répertoire actuel.
...
Lorsque vous utilisez le ZYRAXON TUI, vous pouvez taper `/` suivi d'un nom de commande pour exécuter rapidement des actions. Par exemple:
...
Ajoutez un fournisseur à ZYRAXON. Vous permet de sélectionner parmi les fournisseurs disponibles et d’ajouter leurs clés API.
...
Ceci est séparé de `zyraxon.json`, qui configure le comportement du serveur/d'exécution.
...
- `leader_timeout` - Contrôle la durée pendant laquelle ZYRAXON attend après la leader key. Valeur par défaut : `2000`.
