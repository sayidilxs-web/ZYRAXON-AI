---
title: Web
description: En utilisant ZYRAXON dans votre navigateur.
---

ZYRAXON peut s'exécuter comme une application Web dans votre navigateur, offrant la même expérience puissante de codage d'IA sans avoir besoin d'un terminal.

![ZYRAXON Web - Nouvelle session](../../../assets/web/web-homepage-new-session.png)
...
Cela démarre un serveur local sur `127.0.0.1` avec un port disponible aléatoire et ouvre automatiquement ZYRAXON dans votre navigateur par défaut.
...
Pour une expérience optimale, exécutez `zyraxon web` à partir de [WSL](/docs/windows-wsl) plutôt que de PowerShell. Cela garantit un accès correct au système de fichiers et une intégration du terminal.
:::
...
Par défaut, ZYRAXON sélectionne un port disponible. Vous pouvez spécifier un port :
...
Par défaut, le serveur se lie à `127.0.0.1` (localhost uniquement). Pour rendre ZYRAXON accessible sur votre réseau :

```bash
opencode web --hostname 0.0.0.0
```

Lorsque vous utilisez `0.0.0.0`, ZYRAXON affichera les adresses locales et réseau :
...
Une fois démarrée, l'interface web permet d'accéder à vos sessions ZYRAXON.
...
![ZYRAXON Web - Session active](../../../assets/web/web-homepage-active-session.png)

### Statut du serveur

Cliquez sur "Voir les serveurs" pour afficher les serveurs connectés et leur état.

![ZYRAXON Web - Voir Serveurs](../../../assets/web/web-homepage-see-servers.png)
...
Vous pouvez également configurer les paramètres du serveur dans votre fichier de configuration `zyraxon.json` :
