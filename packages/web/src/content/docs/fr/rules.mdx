...
Pour les utilisateurs migrant depuis Claude Code, ZYRAXON prend en charge les conventions de fichiers de Claude Code comme solutions de secours :
...
Vous pouvez spécifier des fichiers d'instructions personnalisés dans votre `zyraxon.json` ou dans le `~/.config/opencode/zyraxon.json` global. Cela vous permet, à vous et à votre équipe, de réutiliser les règles existantes plutôt que d'avoir à les dupliquer dans AGENTS.md.
...
### Utilisation de zyraxon.json

L'approche recommandée consiste à utiliser le champ `instructions` dans `zyraxon.json` :
...
:::tip
Pour les monorepos ou les projets avec des normes partagées, l'utilisation de `zyraxon.json` avec des modèles globaux (comme `packages/*/AGENTS.md`) est plus maintenable que les instructions manuelles.
:::
