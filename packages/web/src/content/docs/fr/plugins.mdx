---
title: Plugins
description: Écrivez vos propres plugins pour étendre ZYRAXON.
---

Les plugins vous permettent d'étendre ZYRAXON en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de ZYRAXON.
...
1. Configuration globale (`~/.config/opencode/zyraxon.json`)
2. Configuration du projet (`zyraxon.json`)
...
ZYRAXON exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
:::note
Si vous utilisez l'application de bureau ZYRAXON, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Niveaux : `debug`, `info`, `warn`, `error`. Voir la [documentation du SDK](https://zyraxon.ai/docs/sdk) pour plus de détails.
