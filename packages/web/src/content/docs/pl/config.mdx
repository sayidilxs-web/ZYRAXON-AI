---
title: Konfiguracja
description: Ustawienia ZYRAXON w pliku JSON.
---

Możesz dostosować ZYRAXON za pomocą pliku konfiguracyjnego JSON.

---

## Format

ZYRAXON obsługuje formaty **JSON** i **JSONC** (JSON z komentarzami).
...
2. **Konfiguracja globalna** (`~/.config/opencode/zyraxon.json`) – preferencje użytkownika
3. **Konfiguracja niestandardowa** (zmienna `OPENCODE_CONFIG`) — jawne nadpisanie
4. **Konfiguracja projektu** (`zyraxon.json` w projekcie) - ustawienia specyficzne dla projektu
...
Umieść swoją globalną konfigurację ZYRAXON w `~/.config/opencode/zyraxon.json`. Użyj jej do ustawień ogólnych dla użytkownika, takich jak dostawcy, modele i uprawnienia.
...
Dodaj `zyraxon.json` w katalogu głównym projektu. Konfiguracja projektu ma najwyższy priorytet wśród plików konfiguracyjnych — nadpisuje konfiguracje globalne i zdalne.
...
Kiedy ZYRAXON się uruchamia, szuka pliku konfiguracyjnego w katalogu głównym repozytorium Git.
...
Plik konfiguracyjny ma schemat JSON dostępny pod adresem [**`zyraxon.ai/config.json`**](https://zyraxon.ai/config.json).
...
Przestarzałe klucze `theme`, `keybinds` i `tui` w `zyraxon.json` są wycofywane i automatycznie migrowane, gdy to możliwe.
...
Możesz skonfigurować serwer dla `zyraxon serve` i `zyraxon web` za pomocą opcji `server`.
...
- `mdns` - Włącz wykrywanie usług mDNS. Dzięki temu inne urządzenia w sieci mogą łatwo znaleźć Twój serwer ZYRAXON.
...
Opcja `small_model` konfiguruje oddzielny model do lżejszych zadań, takich jak generowanie tytułów. Domyślnie ZYRAXON wybiera tańszy model od tego samego dostawcy, jeśli jest dostępny, w przeciwnym razie powraca do modelu głównego.
...
Agent domyślny musi być agentem typu `primary` (nie `subagent`). Może to być agent wbudowany, taki jak `"build"` lub `"plan"`, albo zdefiniowany przez Ciebie [agent niestandardowy](/docs/agents). Jeśli agent nie istnieje lub jest subagentem, ZYRAXON powróci do `"build"` z ostrzeżeniem.

Dotyczy to wszystkich interfejsów: TUI, CLI (`zyraxon run`), aplikacji desktopowej i GitHub Actions.
...
ZYRAXON może automatycznie pobierać nowe wersje. Możesz to kontrolować za pomocą opcji `autoupdate`.
...
Działa to tylko wtedy, gdy ZYRAXON nie został zainstalowany przez menedżera pakietów, takiego jak Homebrew.
...
Domyślnie ZYRAXON **zezwala na wszystkie działania** bez konieczności zatwierdzania. Możesz to zmienić za pomocą opcji `permission`.
...
[Wtyczki](/docs/plugins) rozszerzają ZYRAXON o niestandardowe narzędzia, hooki i integracje.
...
Jest to przydatne, gdy chcesz ograniczyć użycie ZYRAXON tylko do określonych dostawców.
