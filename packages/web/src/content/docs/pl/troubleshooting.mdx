...
Możesz ustawić poziom dziennika za pomocą opcji wiersza poleceń `--log-level`, aby uzyskać bardziej szczegółowe informacje debugowania. Na przykład `zyraxon --log-level DEBUG`.
...
- Tylko macOS: menu `ZYRAXON` -> **Załaduj ponownie przeglądarkę internetową** (pomaga, jeśli interfejs użytkownika jest pusty/zawieszony).
...
- **macOS/Linux**: `~/.config/opencode/zyraxon.jsonc` (lub `~/.config/opencode/zyraxon.json`)
- **macOS/Linux** (starsze instalacje): `~/.local/share/opencode/zyraxon.jsonc`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\opencode\zyraxon.jsonc`
...
Jeśli Twój `zyraxon.json(c)` zawiera sekcję `server`, tymczasowo usuń ją i uruchom ponownie aplikację komputerową.
...
3. Upewnij się, że masz najnowszą wersję z `zyraxon upgrade`
...
Aby dowiedzieć się, do jakich modeli masz dostęp, uruchom `zyraxon models`
