...
Możesz określić niestandardowe pliki instrukcji w swoim `zyraxon.json` lub globalnym `~/.config/opencode/zyraxon.json`. Dzięki temu Ty i Twój zespół możecie ponownie wykorzystać istniejące reguły, zamiast duplikować je w pliku AGENTS.md.
...
### Używanie zyraxon.json

Zalecanym podejściem jest użycie pola `instructions` w `zyraxon.json`:
...
:::tip
W przypadku monorepo lub projektów ze wspólnymi standardami użycie `zyraxon.json` ze wzorami globalnymi (takimi jak `packages/*/AGENTS.md`) jest łatwiejsze w utrzymaniu niż instrukcje ręczne.
:::
