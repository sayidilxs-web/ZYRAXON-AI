...
You can add external tools to ZYRAXON using the _Model Context Protocol_, or MCP. ZYRAXON supports both local and remote servers.
...
You can define MCP servers in your [ZYRAXON Config](https://zyraxon.ai/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
ZYRAXON automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, ZYRAXON will:
...
If the server requires authentication, ZYRAXON will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `zyraxon mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, ZYRAXON will store the tokens securely in `~/.local/share/opencode/mcp-auth.json`.
...
Your MCPs are available as tools in ZYRAXON, alongside built-in tools. So you can manage them through the ZYRAXON config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect ZYRAXON to your Sentry account.
