---
title: Policies
description: Control which configured resources ZYRAXON may use.
---

Policies control whether ZYRAXON may perform an action on a named resource. This feature is experimental and is configured with the `experimental.policies` array in `zyraxon.json`.

Policies are separate from [permissions](/docs/permissions). Permissions control what tools can do during a session, while policies control whether ZYRAXON may use a resource such as an LLM provider.
...
ZYRAXON currently supports one policy action:
