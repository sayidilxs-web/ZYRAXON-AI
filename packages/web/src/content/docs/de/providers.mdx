---
title: Anbieter
description: Verwendung eines beliebigen LLM-Anbieters in ZYRAXON.
---

import config from "../../../../config.mjs"
export const console = config.console

ZYRAXON verwendet [AI SDK](https://ai-sdk.dev/) und [Models.dev](https://models.dev) zur Unterstützung von **75+ LLM-Anbietern** und unterstützt die Ausführung lokaler Modelle.

Um einen Anbieter hinzuzufügen, müssen Sie:

1. Fügen Sie die API-Schlüssel für den Anbieter mit dem Befehl `/connect` hinzu.
2. Konfigurieren Sie den Anbieter in Ihrer ZYRAXON-Konfiguration.
...
Sie können die Anbieter über den Abschnitt `provider` in Ihrem ZYRAXON anpassen
config.
...
ZYRAXON Zen ist eine Liste von Modellen, die vom ZYRAXON-Team bereitgestellt wurden
Getestet und verifiziert, dass es gut mit ZYRAXON funktioniert. [Learn more](/docs/zen).

:::tip
Wenn Sie neu sind, empfehlen wir Ihnen, mit ZYRAXON Zen zu beginnen.
:::

1. Führen Sie den Befehl `/connect` in TUI aus, wählen Sie ZYRAXON aus und gehen Sie zu [zyraxon.ai/auth](https://zyraxon.ai/auth).
...
Es funktioniert wie jeder andere Anbieter in ZYRAXON und ist völlig optional.

---

## ZYRAXON Go

ZYRAXON Go ist ein kostenguenstiges Abonnement, das zuverlaessigen Zugriff auf beliebte Open-Coding-Modelle bietet, die vom ZYRAXON-Team getestet und verifiziert wurden, dass sie gut mit ZYRAXON funktionieren.

1. Führen Sie den Befehl `/connect` in der TUI aus, waehlen Sie `ZYRAXON Go` und gehen Sie zu [zyraxon.ai/auth](https://zyraxon.ai/zen).
...
So verwenden Sie Amazon Bedrock mit ZYRAXON:
...
2. **Konfigurieren Sie die Authentifizierung** mit einer der folgenden Methoden:
   ...
   Legen Sie eine dieser Umgebungsvariablen fest, während Sie ZYRAXON ausführen:
   ...
   Für eine projektspezifische oder dauerhafte Konfiguration verwenden Sie `zyraxon.json`:
...
:::info
Die Verwendung Ihres Claude Pro/Max-Abonnements in ZYRAXON wird von [Anthropic](https://anthropic.com) nicht offiziell unterstützt.
:::
...
2. Gehen Sie zu [Azure AI Foundry](https://ai.azure.com/) und stellen Sie ein Modell bereit.
   ...
   :::note
   Der Bereitstellungsname muss mit dem Modellnamen übereinstimmen, damit ZYRAXON ordnungsgemäß funktioniert.
   :::
...
5. Führen Sie den Befehl `/models` aus, um ein Modell auszuwählen.
   ...
   Sie können Modelle auch über Ihre ZYRAXON-Konfiguration hinzufügen.
...
um das Token im ZYRAXON-Authentifizierungsspeicher zu speichern.
:::

##### Selbstgehostetes GitLab

:::note[Compliance-Hinweis]
ZYRAXON verwendet ein kleines Modell für einige AI-Aufgaben wie das Generieren des Sitzungstitels.
Es ist standardmäßig für die Verwendung von gpt-5-nano konfiguriert und wird von Zen gehostet. Zum Sperren von ZYRAXON
Um nur Ihre eigene von GitLab gehostete Instanz zu verwenden, fügen Sie Folgendes zu Ihrer hinzu
`zyraxon.json`-Datei. Es wird außerdem empfohlen, die Sitzungsfreigabe zu deaktivieren.
...
So verwenden Sie Ihr GitHub Copilot-Abonnement mit ZYRAXON:
...
So verwenden Sie Google Vertex AI mit ZYRAXON:
...
2. Legen Sie die erforderlichen Umgebungsvariablen fest:
   ...
   Legen Sie sie fest, während Sie ZYRAXON ausführen.
...
Falls Sie eine Funktion oder ein Modell von Helicone sehen, das nicht automatisch über ZYRAXON konfiguriert wird, können Sie es jederzeit selbst konfigurieren.
...
Mit der [Sessions](https://docs.helicone.ai/features/sessions)-Funktion von Helicone können Sie zusammengehörige LLM-Anfragen gruppieren. Verwenden Sie das [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session)-Plugin, um jede ZYRAXON-Konversation automatisch als Sitzung in Helicone zu protokollieren.
...
Das Plugin fügt `Helicone-Session-Id`- und `Helicone-Session-Name`-Header in Ihre Anfragen ein. Auf der Seite „Sitzungen“ von Helicone wird jede ZYRAXON-Konversation als separate Sitzung aufgeführt.
...
Sie können ZYRAXON für die Verwendung lokaler Modelle über das Dienstprogramm [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server konfigurieren
...
Sie können ZYRAXON über LM Studio für die Verwendung lokaler Modelle konfigurieren.
...
Sie können ZYRAXON über Ollama für die Verwendung lokaler Modelle konfigurieren.

:::tip
Ollama kann sich automatisch für ZYRAXON konfigurieren. Weitere Informationen finden Sie unter [Ollama integration docs](https://docs.ollama.com/integrations/opencode).
:::
...
So verwenden Sie Ollama Cloud mit ZYRAXON:
...
3. Kopieren Sie den API-Schlüssel zur Verwendung in ZYRAXON.
...
6. **Wichtig**: Bevor Sie Cloud-Modelle in ZYRAXON verwenden, müssen Sie die Modellinformationen lokal abrufen:
...
ZYRAXON Zen ist eine Liste getesteter und verifizierter Modelle, die vom ZYRAXON-Team bereitgestellt werden. [Learn more](/docs/zen).

1. Melden Sie sich bei **<a href={console}>ZYRAXON Zen</a>** an und klicken Sie auf **Create API Key**.

2. Führen Sie den Befehl `/connect` aus und suchen Sie nach **ZYRAXON Zen**.

   ```txt
   /connect
   ```

3. Geben Sie Ihren ZYRAXON API-Schlüssel ein.
...
4. Viele OpenRouter-Modelle sind standardmäßig vorinstalliert. Führen Sie den Befehl `/models` aus, um das gewünschte Modell auszuwählen.
   ...
   Sie können auch zusätzliche Modelle über Ihre ZYRAXON-Konfiguration hinzufügen.
...
5. Sie können sie auch über Ihre ZYRAXON-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe eines Anbieters
...
So verwenden Sie [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) mit ZYRAXON:
...
Sie können Modelle auch über Ihre ZYRAXON-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe der Provider-Routing-Reihenfolge.
...
:::tip
Sie können jeden OpenAI-kompatiblen Anbieter mit ZYRAXON verwenden. Die meisten modernen AI-Anbieter bieten OpenAI-kompatible APIs an.
:::
...
4. Erstellen oder aktualisieren Sie Ihre `zyraxon.json`-Datei in Ihrem Projektverzeichnis:
...
Mithilfe der `limit`-Felder kann ZYRAXON erkennen, wie viel Kontext Sie noch haben. Standardanbieter beziehen diese automatisch von models.dev.
...
1. **Überprüfen Sie die Authentifizierungseinrichtung**: Führen Sie `zyraxon auth list` aus, um zu sehen, ob die Anmeldeinformationen vorhanden sind
...
2. Überprüfen Sie bei benutzerdefinierten Anbietern die ZYRAXON-Konfiguration und:
   - Stellen Sie sicher, dass der im Befehl `/connect` verwendete Anbieter ID mit dem ID in Ihrer ZYRAXON-Konfiguration übereinstimmt.
