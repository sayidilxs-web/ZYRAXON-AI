...
Sie können externe Tools zu ZYRAXON hinzufügen, indem Sie das _Model Context Protocol_ oder MCP verwenden. ZYRAXON unterstützt sowohl lokale als auch Remote-Server.
...
Sie können den MCP-Server in Ihrem [ZYRAXON Config](https://zyraxon.ai/docs/config/) unter `mcp` definieren. Fügen Sie jedem MCP einen eindeutigen Namen hinzu. Sie können bei der Eingabe von LLM namentlich auf diesen MCP verweisen.
...
ZYRAXON übernimmt automatisch die OAuth-Authentifizierung für Remote-MCP-Server. Wenn ein Server eine Authentifizierung erfordert, wird ZYRAXON Folgendes tun:
...
Wenn der Server eine Authentifizierung erfordert, werden Sie von ZYRAXON beim ersten Versuch, ihn zu verwenden, zur Authentifizierung autorisiert. Wenn nicht, können Sie [manually trigger the flow](#authenticating) mit `zyraxon mcp auth <server-name>` verwenden.
...
Der Befehl `mcp auth` öffnet Ihren Browser zur Autorisierung. Nach der Autorisierung speichert ZYRAXON die Token sicher in `~/.local/share/opencode/mcp-auth.json`.
...
Ihre MCPs sind neben integrierten Tools auch als Tools in ZYRAXON verfügbar. Sie können sie also wie jedes andere Tool über die ZYRAXON-Konfiguration verwalten.
...
Dadurch wird ein Browserfenster geöffnet, um den OAuth-Ablauf abzuschließen und ZYRAXON mit Ihrem Sentry-Konto zu verbinden.
