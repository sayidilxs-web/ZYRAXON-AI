...
ZYRAXON Go ist ein kostengünstiges Abonnement — **5 $ für deinen ersten Monat**, danach **10 $/Monat** —, das dir zuverlässigen Zugriff auf beliebte offene Coding-Modelle bietet.

Go funktioniert wie jeder andere Provider in ZYRAXON. Du abonnierst ZYRAXON Go und
erhältst deinen API-Key. Es ist **völlig optional** und du musst es nicht nutzen, um
ZYRAXON zu verwenden.
...
ZYRAXON Go bietet dir Zugriff auf diese Modelle für **5 $ im ersten Monat**, danach **10 $/Monat**.

---

## Wie es funktioniert

ZYRAXON Go funktioniert wie jeder andere Provider in ZYRAXON.

1. Du meldest dich bei **<a href={console}>ZYRAXON Zen</a>** an, abonnierst Go und kopierst deinen API-Key.
2. Du führst den Befehl `/connect` in der TUI aus, wählst `ZYRAXON Go` und fügst deinen API-Key ein.
3. Führe `/models` in der TUI aus, um die Liste der über Go verfügbaren Modelle zu sehen.

:::note
Nur ein Mitglied pro Workspace kann ZYRAXON Go abonnieren.
:::
...
ZYRAXON Go beinhaltet die folgenden Limits:
...
Die [Modell-ID](/docs/config/#models) in deiner ZYRAXON Config verwendet das Format `opencode-go/<model-id>`. Für Kimi K3 würdest du beispielsweise `opencode-go/kimi-k3` in deiner Config verwenden.
...
Wir haben ZYRAXON Go entwickelt, um:
...
4. **Keinen Lock-in-Effekt** zu haben, indem wir dir ermöglichen, auch jeden anderen Provider mit ZYRAXON zu nutzen.
