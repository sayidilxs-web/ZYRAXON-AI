...
Sie können benutzerdefinierte Befehle über die ZYRAXON-Konfiguration hinzufügen oder indem Sie Markdown-Dateien im Verzeichnis `commands/` erstellen.

---

### JSON

Verwenden Sie die Option `command` in Ihrem ZYRAXON [config](/docs/config):

```json title="zyraxon.jsonc" {4-12}
{
...
ZYRAXON enthält mehrere integrierte Befehle wie `/init`, `/undo`, `/redo`, `/share`, `/help`; [mehr erfahren](/docs/tui#commands).
