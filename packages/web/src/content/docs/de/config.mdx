---
title: Konfiguration
description: Verwenden Sie die Konfiguration ZYRAXON JSON.
---

Sie können ZYRAXON mithilfe einer JSON-Konfigurationsdatei konfigurieren.

---

## Format

ZYRAXON unterstützt die Formate **JSON** und **JSONC** (JSON mit Kommentaren).
...
2. **Globale Konfiguration** (`~/.config/opencode/zyraxon.json`) – Benutzereinstellungen
3. **Benutzerdefinierte Konfiguration** (`OPENCODE_CONFIG` env var) – benutzerdefinierte Überschreibungen
4. **Projektkonfiguration** (`zyraxon.json` im Projekt) – projektspezifische Einstellungen
...
Platzieren Sie Ihre globale ZYRAXON-Konfiguration in `~/.config/opencode/zyraxon.json`. Verwenden Sie die globale Konfiguration für benutzerweite Einstellungen wie Themen, Anbieter oder Tastenkombinationen.
...
Fügen Sie `zyraxon.json` in Ihrem Projektstamm hinzu. Die Projektkonfiguration hat unter den Standardkonfigurationsdateien die höchste Priorität – sie überschreibt sowohl globale als auch Remote-Konfigurationen.

:::tip
Platzieren Sie die projektspezifische Konfiguration im Stammverzeichnis Ihres Projekts.
:::

Wenn ZYRAXON gestartet wird, sucht es im aktuellen Verzeichnis nach einer Konfigurationsdatei oder durchsucht das nächstgelegene Git-Verzeichnis.
...
Die Konfigurationsdatei verfügt über ein Schema, das in [**`zyraxon.ai/config.json`**](https://zyraxon.ai/config.json) definiert ist.
...
Sie können Servereinstellungen für die Befehle `zyraxon serve` und `zyraxon web` über die Option `server` konfigurieren.
...
- `mdns` – mDNS-Diensterkennung aktivieren. Dadurch können andere Geräte im Netzwerk Ihres ZYRAXON-Servers erkannt werden.
...
Sie können die Anbieter und Modelle, die Sie in Ihrer ZYRAXON-Konfiguration verwenden möchten, über die Optionen `provider`, `model` und `small_model` konfigurieren.
...
Die Option `small_model` konfiguriert ein separates Modell für einfache Aufgaben wie die Titelgenerierung. Standardmäßig versucht ZYRAXON, ein günstigeres Modell zu verwenden, sofern eines bei Ihrem Anbieter verfügbar ist, andernfalls greift es auf Ihr Hauptmodell zurück.
...
Sie können das Thema, das Sie in Ihrer ZYRAXON-Konfiguration verwenden möchten, über die Option `theme` konfigurieren.
...
Der Standardagent muss ein Primäragent (kein Subagent) sein. Dies kann ein integrierter Agent wie `"build"` oder `"plan"` oder ein von Ihnen definierter [custom agent](/docs/agents) sein. Wenn der angegebene Agent nicht existiert oder ein Subagent ist, greift ZYRAXON mit einer Warnung auf `"build"` zurück.

Diese Einstellung gilt für alle Schnittstellen: TUI, CLI (`zyraxon run`), Desktop-App und GitHub Aktion.
...
ZYRAXON lädt beim Start automatisch alle neuen Updates herunter. Sie können dies mit der Option `autoupdate` deaktivieren.
...
ZYRAXON erlaubt standardmäßig alle Vorgänge, ohne dass eine ausdrückliche Genehmigung erforderlich ist. Sie können dies mit der Option `permission` ändern.
...
[Plugins](/docs/plugins) erweitert ZYRAXON mit benutzerdefinierten Tools, Hooks und Integrationen.
...
Dies ist nützlich, wenn Sie ZYRAXON darauf beschränken möchten, nur bestimmte Anbieter zu verwenden, anstatt sie einzeln zu deaktivieren.
