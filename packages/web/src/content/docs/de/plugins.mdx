---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um ZYRAXON zu erweitern.
---

Mit Plugins können Sie ZYRAXON erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von ZYRAXON zu ändern.
...
1. Globale Konfiguration (`~/.config/opencode/zyraxon.json`)
2. Projektkonfiguration (`zyraxon.json`)
...
ZYRAXON führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
- `client`: Ein ZYRAXON-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie ZYRAXON erweitern können.
...
:::note
Wenn Sie die ZYRAXON-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass ZYRAXON `.env`-Dateien liest:
...
Plugins können ZYRAXON auch benutzerdefinierte Tools hinzufügen:

```ts title=".opencode/plugins/custom-tools.ts"
import { type Plugin, tool } from "@zyraxon-ai/plugin"
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das ZYRAXON aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für ZYRAXON zur Verfügung.
...
Ebenen: `debug`, `info`, `warn`, `error`. Weitere Informationen finden Sie unter [SDK documentation](https://zyraxon.ai/docs/sdk).
