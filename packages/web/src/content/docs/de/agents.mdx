...
Es gibt zwei Arten von Agenten in ZYRAXON; Primäragenten und Subagenten.
...
ZYRAXON verfügt über zwei integrierte Primäragenten: **Build** und **Plan**. Auch
...
Konfigurieren Sie Agenten in Ihrer `zyraxon.json`-Konfigurationsdatei:
...
Wenn keine Temperatur angegeben ist, verwendet ZYRAXON modellspezifische Standardwerte; typischerweise 0 für die meisten Modelle, 0.55 für Qwen-Modelle.
...
Dieser Pfad ist relativ zum Speicherort der Konfigurationsdatei. Dies funktioniert sowohl für die globale ZYRAXON-Konfiguration als auch für die projektspezifische Konfiguration.
...
Die Modell-ID in Ihrer ZYRAXON-Konfiguration verwendet das Format `provider/model-id`. Wenn Sie beispielsweise [ZYRAXON Zen](/docs/zen) verwenden, würden Sie `opencode/gpt-5.1-codex` für GPT 5.1 Codex verwenden.
...
:::tip
Führen Sie `zyraxon models` aus, um eine Liste der verfügbaren Modelle anzubieten.
:::
...
:::tip
Haben Sie einen Agenten, den Sie gerne teilen möchten? [Reichen Sie einen PR ein](https://github.com/onelpawarai/ZYRAXON-AI).
:::
