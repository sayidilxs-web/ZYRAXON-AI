...
Wenn ZYRAXON Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
Mit `--log-level` bekommst du detailliertere Diagnoseinfos, z. B. `zyraxon --log-level DEBUG`.
...
ZYRAXON Desktop startet im Hintergrund einen lokalen ZYRAXON-Server (`opencode-cli`-Sidecar).
...
- Nur macOS: `ZYRAXON`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
- **macOS/Linux**: `~/.config/opencode/zyraxon.jsonc` (oder `~/.config/opencode/zyraxon.json`)
- **macOS/Linux** (aeltere Installationen): `~/.local/share/opencode/zyraxon.jsonc`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\opencode\zyraxon.jsonc`
...
ZYRAXON kann auch lokale Plugins von der Festplatte laden. Verschiebe diese voruebergehend (oder benenne den Ordner um) und starte die Desktop-App neu:
...
Wenn das Deaktivieren von Plugins nicht hilft (oder eine Plugin-Installation haengt), leere den Cache, damit ZYRAXON ihn neu aufbauen kann.

1. Beende ZYRAXON Desktop komplett.
...
3. Starte ZYRAXON Desktop neu.

---

### Server-Verbindungsprobleme beheben

ZYRAXON Desktop kann entweder einen eigenen lokalen Server starten (Standard) oder sich mit einer konfigurierten Server-URL verbinden.
...
Wenn deine `zyraxon.json(c)` einen `server`-Abschnitt enthaelt, entferne ihn voruebergehend und starte die Desktop-App neu.
...
Unter Windows benoetigt ZYRAXON Desktop die Microsoft Edge **WebView2 Runtime**. Wenn die App ein leeres Fenster zeigt oder nicht startet, installiere/aktualisiere WebView2 und versuche es erneut.

---

### Windows: Allgemeine Performance-Probleme

Wenn du langsame Performance, Dateizugriffsprobleme oder Terminal-Probleme unter Windows hast, versuche [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL bietet eine Linux-Umgebung, die nahtloser mit ZYRAXON-Features funktioniert.

---

### Benachrichtigungen werden nicht angezeigt

ZYRAXON Desktop zeigt Systembenachrichtigungen nur wenn:

- Benachrichtigungen fuer ZYRAXON in den OS-Einstellungen aktiviert sind, und
...
1. Beende ZYRAXON Desktop.
2. Finde und loesche diese Dateien (im App-Data-Verzeichnis von ZYRAXON Desktop):
...
Wenn du Probleme mit ZYRAXON hast:
...
Um zu sehen, auf welche Modelle du Zugriff hast, fuehre `zyraxon models` aus.
