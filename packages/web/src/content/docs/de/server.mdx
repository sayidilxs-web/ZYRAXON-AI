...
Der Befehl `zyraxon serve` startet einen headless HTTP-Server.
...
Der Benutzername ist standardmaessig `opencode`, kann aber mit `OPENCODE_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `zyraxon serve` und `zyraxon web`.
...
Mit `zyraxon serve` startest du einen eigenstaendigen Server.
Laeuft bereits eine TUI, startet `zyraxon serve` trotzdem eine neue Serverinstanz.
...
Dieses Muster nutzen auch die ZYRAXON-[IDE](/docs/ide)-Plugins.
