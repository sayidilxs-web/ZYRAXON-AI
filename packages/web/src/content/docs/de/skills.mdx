...
Agent Skills erlauben ZYRAXON, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
ZYRAXON sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft ZYRAXON vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
...
ZYRAXON listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `zyraxon.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `zyraxon.json`):
