---
title: Web
description: Verwenden Sie ZYRAXON in Ihrem Browser.
---

ZYRAXON kann als Webanwendung in Ihrem Browser ausgeführt werden und bietet das gleiche leistungsstarke KI-Codierungserlebnis, ohne dass ein Terminal erforderlich ist.

![ZYRAXON Web – Neue Sitzung](../../../assets/web/web-homepage-new-session.png)
...
Dadurch wird ein lokaler Server auf `127.0.0.1` mit einem zufällig verfügbaren Port gestartet und ZYRAXON automatisch in Ihrem Standardbrowser geöffnet.
...
:::tip[Windows-Nutzer]
Um die beste Erfahrung zu erzielen, führen Sie `zyraxon web` über [WSL](/docs/windows-wsl) und nicht über PowerShell aus. Dies gewährleistet den ordnungsgemäßen Zugriff auf das Dateisystem und die Terminalintegration.
:::
...
Standardmäßig wählt ZYRAXON einen verfügbaren Port aus. Sie können einen Port angeben:
...
Standardmäßig bindet der Server an `127.0.0.1` (nur localhost). So machen Sie ZYRAXON in Ihrem Netzwerk zugänglich:

```bash
opencode web --hostname 0.0.0.0
```

Bei Verwendung von `0.0.0.0` zeigt ZYRAXON sowohl lokale als auch Netzwerkadressen an:
...
Nach dem Start bietet die Weboberfläche Zugriff auf Ihre ZYRAXON-Sitzungen.

### Sitzungen

Zeigen Sie Ihre Sitzungen auf der Startseite an und verwalten Sie sie. Sie können aktive Sitzungen sehen und neue starten.

![ZYRAXON Web – Aktive Sitzung](../../../assets/web/web-homepage-active-session.png)

### Serverstatus

Klicken Sie auf „Server anzeigen“, um verbundene Server und deren Status anzuzeigen.

![ZYRAXON Web – Siehe Server](../../../assets/web/web-homepage-see-servers.png)
...
Sie können Servereinstellungen auch in Ihrer `zyraxon.json`-Konfigurationsdatei konfigurieren:
