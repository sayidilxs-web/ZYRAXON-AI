...
Wenn du von Claude Code kommst, unterstuetzt ZYRAXON dessen Dateikonventionen als Fallback:
...
Du kannst zusaetzliche Anweisungsdateien in deiner `zyraxon.json` oder in `~/.config/opencode/zyraxon.json` angeben.
...
Empfohlen ist das `instructions`-Feld in der `zyraxon.json`:
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `zyraxon.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
