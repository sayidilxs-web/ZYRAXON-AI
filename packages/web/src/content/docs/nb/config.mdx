...
Du kan konfigurere ZYRAXON ved å bruke en JSON konfigurasjonsfil.

---

## Format

ZYRAXON støtter både **JSON** og **JSONC** (JSON med kommentarer) formater.
...
2. **Global konfigurasjon** (`~/.config/opencode/zyraxon.json`) - brukerinnstillinger
3. **Egendefinert konfigurasjon** (`OPENCODE_CONFIG` env var) - egendefinerte overstyringer
4. **Prosjektkonfigurasjon** (`zyraxon.json` i prosjekt) - prosjektspesifikke innstillinger
...
Plasser din globale ZYRAXON-konfigurasjon i `~/.config/opencode/zyraxon.json`. Bruk global konfigurasjon for brukerspesifikke preferanser som temaer, leverandører eller nøkkelbindinger.
...
Legg til `zyraxon.json` i prosjektroten din. Prosjektkonfigurasjon har den høyeste prioritet blant standard config-filer - den overstyrer både globale og eksterne konfigurasjoner.

:::tip
Plasser prosjektspesifikk konfigurasjon i roten til prosjektet ditt.
:::

Når ZYRAXON starter opp, ser den etter en konfigurasjonsfil i gjeldende katalog eller går opp til nærmeste Git-katalog.
...
Konfigurasjonsfilen har et skjema som er definert i [**`zyraxon.ai/config.json`**](https://zyraxon.ai/config.json).
...
Du kan konfigurere serverinnstillinger for kommandoene `zyraxon serve` og `zyraxon web` gjennom alternativet `server`.
...
- `mdns` - Aktiver mDNS-tjenesteoppdagelse. Dette lar andre enheter på nettverket oppdage ZYRAXON-serveren din.
...
Du kan konfigurere leverandørene og modellene du vil bruke i ZYRAXON-konfigurasjonen gjennom alternativene `provider`, `model` og `small_model`.
...
Alternativet `small_model` konfigurerer en egen modell for lette oppgaver som tittelgenerering. Som standard prøver ZYRAXON å bruke en rimeligere modell hvis en er tilgjengelig fra leverandøren din, ellers faller den tilbake til hovedmodellen din.
...
Standardagenten må være en primæragent (ikke en underagent). Dette kan være en innebygd agent som `"build"` eller `"plan"`, eller en [egendefinert agent](/docs/agents) du har definert. Hvis den angitte agenten ikke eksisterer eller er en subagent, vil ZYRAXON falle tilbake til `"build"` med en advarsel.

Denne innstillingen gjelder for alle grensesnitt: TUI, CLI (`zyraxon run`), skrivebordsapp og GitHub Action.
...
ZYRAXON vil automatisk laste ned eventuelle nye oppdateringer når den starter opp. Du kan deaktivere dette med alternativet `autoupdate`.
...
Som standard **tillater ZYRAXON alle operasjoner** uten å kreve eksplisitt godkjenning. Du kan endre dette ved å bruke alternativet `permission`.
...
[Plugins](/docs/plugins) utvider ZYRAXON med tilpassede verktøy, kroker og integrasjoner.
...
Dette er nyttig når du vil begrense ZYRAXON til kun å bruke spesifikke leverandører i stedet for å deaktivere dem én etter én.
