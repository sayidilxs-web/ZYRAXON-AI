...
For å feilsøke problemer med ZYRAXON, start med å sjekke loggene og lokale data den lagrer på disken.
...
Du kan angi loggnivået med kommandolinjealternativet `--log-level` for å få mer detaljert feilsøkingsinformasjon. For eksempel `zyraxon --log-level DEBUG`.

---

## Lagring

ZYRAXON lagrer øktdata og andre applikasjonsdata på disken på:
...
ZYRAXON Desktop kjører en lokal ZYRAXON-server (`opencode-cli` sidevognen) i bakgrunnen. De fleste problemene er forårsaket av en plugin som fungerer dårlig, en ødelagt cache eller en dårlig serverinnstilling.

### Raske sjekker

- Avslutt og start appen på nytt.
- Hvis appen viser en feilskjerm, klikker du på **Start på nytt** og kopierer feildetaljene.
- Bare macOS: `ZYRAXON`-meny -> **Last nettvisning på nytt** (hjelper hvis UI er tom/frosset).
...
- **macOS/Linux**: `~/.config/opencode/zyraxon.jsonc` (eller `~/.config/opencode/zyraxon.json`)
- **macOS/Linux** (eldre installasjoner): `~/.local/share/opencode/zyraxon.jsonc`
- **Windows**: Trykk `WIN+R` og lim inn `%USERPROFILE%\.config\opencode\zyraxon.jsonc`
...
ZYRAXON kan også laste lokale plugins fra disken. Flytt disse midlertidig ut av veien (eller gi nytt navn til mappen) og start skrivebordsappen på nytt:
...
Hvis deaktivering av plugins ikke hjelper (eller en plugin-installasjon sitter fast), tøm hurtigbufferen slik at ZYRAXON kan gjenoppbygge den.

1. Avslutt ZYRAXON Desktop helt.
...
3. Start ZYRAXON Desktop på nytt.

---

### Løs problemer med servertilkobling

ZYRAXON Desktop kan enten starte sin egen lokale server (standard) eller koble til en server URL du har konfigurert.
...
Hvis `zyraxon.json(c)` inneholder en `server`-del, fjern den midlertidig og start skrivebordsappen på nytt.
...
På Windows krever ZYRAXON Desktop Microsoft Edge **WebView2 Runtime**. Hvis appen åpnes i et tomt vindu eller ikke starter, installer/oppdater WebView2 og prøv igjen.

---

### Windows: Generelle ytelsesproblemer

Hvis du opplever treg ytelse, problemer med filtilgang eller terminalproblemer på Windows, kan du prøve å bruke [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL gir et Linux-miljø som fungerer mer sømløst med funksjonene til ZYRAXON.

---

### Varsler vises ikke

ZYRAXON Desktop viser bare systemvarsler når:

- varsler er aktivert for ZYRAXON i OS-innstillingene dine, og
...
1. Avslutt ZYRAXON Desktop.
2. Finn og slett disse filene (de finnes i ZYRAXON Desktop-appens datakatalog):
...
Hvis du støter på API-anropsfeil, kan dette skyldes utdaterte provider-pakker. ZYRAXON installerer dynamisk provider-pakker (OpenAI, Anthropic, Google, etc.) etter behov og cacher dem lokalt.
...
2. Start ZYRAXON på nytt for å installere de nyeste provider-pakkene på nytt

Dette vil tvinge ZYRAXON til å laste ned de nyeste versjonene av provider-pakkene, som ofte løser kompatibilitetsproblemer med modellparametere og API-endringer.
...
ZYRAXON vil oppdage om du bruker Wayland og foretrekker `wl-clipboard`, ellers vil den prøve å finne utklippstavle-verktøy i rekkefølgen: `xclip` og `xsel`.
