...
description: Skriv dine egne programtillegg for å utvide ZYRAXON.
---

Plugins lar deg utvide ZYRAXON ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til ZYRAXON.
...
1. Global konfigurasjon (`~/.config/opencode/zyraxon.json`)
2. Prosjektkonfigurasjon (`zyraxon.json`)
...
ZYRAXON kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En ZYRAXON SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide ZYRAXON.
...
:::note
Hvis du bruker ZYRAXON-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre ZYRAXON fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til ZYRAXON:
...
`tool`-hjelperen lager et tilpasset verktøy som ZYRAXON kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for ZYRAXON sammen med innebygde verktøy.
...
Nivåer: `debug`, `info`, `warn`, `error`. Se [SDK dokumentasjon](https://zyraxon.ai/docs/sdk) for detaljer.
