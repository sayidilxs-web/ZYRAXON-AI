...
ZYRAXON støtter [Agent Client Protocol](https://agentclientprotocol.com) eller (ACP), slik at du kan bruke den direkte i kompatible editorer og IDE-er.
...
For å bruke ZYRAXON via ACP, konfigurer redigeringsprogrammet til å kjøre kommandoen `zyraxon acp`.

Kommandoen starter ZYRAXON som en ACP-kompatibel underprosess som kommuniserer med editoren din over JSON-RPC via stdio.
...
For å åpne den, bruk den nye 'ZYRAXON'-agenten i AI Chat-agentvelgeren.
...
For å bruke ZYRAXON som en ACP-agent i [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), legg til følgende i Neovim-konfigurasjonen:
...
Denne konfigurasjonen setter opp CodeCompanion.nvim til å bruke ZYRAXON som ACP-agent for chat.
...
ZYRAXON fungerer på samme måte via ACP som i terminalen. Alle funksjoner støttes:
...
- MCP servere konfigurert i ZYRAXON-konfigurasjonen
