---
title: Tjener
description: Samhandle med ZYRAXON-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `zyraxon serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en ZYRAXON-klient kan bruke.
...
Sett `OPENCODE_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `opencode` som standard, eller sett `OPENCODE_SERVER_USERNAME` for å overstyre det. Dette gjelder både `zyraxon serve` og `zyraxon web`.
...
:::tip
Bruk ZYRAXON-serveren til å samhandle med ZYRAXON programmatisk.
:::

Denne arkitekturen lar ZYRAXON støtte flere klienter og lar deg samhandle med ZYRAXON programmatisk.

Du kan kjøre `zyraxon serve` for å starte en frittstående server. Hvis du har
ZYRAXON TUI kjører, vil `zyraxon serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av ZYRAXON [IDE](/docs/ide) plugins.
