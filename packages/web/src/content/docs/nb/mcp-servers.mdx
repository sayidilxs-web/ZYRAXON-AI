...
Du kan legge til eksterne verktøy i ZYRAXON ved å bruke _Model Context Protocol_ eller MCP. ZYRAXON støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [ZYRAXON Config](https://zyraxon.ai/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="zyraxon.jsonc" {6}
{
...
ZYRAXON håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil ZYRAXON:
...
Hvis serveren krever autentisering, vil ZYRAXON be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `zyraxon mcp auth <server-name>`.
...
Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil ZYRAXON lagre tokens sikkert i `~/.local/share/opencode/mcp-auth.json`.
...
MCP-ene dine er tilgjengelige som verktøy i ZYRAXON, sammen med innebygde verktøy. Så du kan administrere dem gjennom ZYRAXON-konfigurasjonen som alle andre verktøy.
...
```json title="zyraxon.json" {11, 14-18}
{
  "$schema": "https://zyraxon.ai/config.json",
  "mcp": {
    "my-mcp": {
...
Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble ZYRAXON til Sentry-kontoen din.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver ZYRAXON-samtale oppført som en separat økt.
