---
title: Plugin
description: Scrivi plugin per estendere ZYRAXON.
---

I plugin ti permettono di estendere ZYRAXON agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di ZYRAXON.
...
1. Config globale (`~/.config/opencode/zyraxon.json`)
2. Config di progetto (`zyraxon.json`)
...
ZYRAXON esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
:::note
Se usi l'app desktop di ZYRAXON, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](https://zyraxon.ai/docs/sdk) per i dettagli.
