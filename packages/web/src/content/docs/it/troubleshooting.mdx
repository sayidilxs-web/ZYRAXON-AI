...
Per diagnosticare problemi con ZYRAXON, inizia controllando i log e i dati locali che salva su disco.
...
Puoi impostare il livello di log con l'opzione a riga di comando `--log-level` per ottenere informazioni di debug piu dettagliate. Per esempio: `zyraxon --log-level DEBUG`.
...
ZYRAXON Desktop esegue in background un server locale di ZYRAXON (il sidecar `opencode-cli`). La maggior parte dei problemi e causata da un plugin che si comporta male, da una cache corrotta o da un'impostazione del server errata.
...
- Solo macOS: menu `ZYRAXON` -> **Reload Webview** (utile se l'interfaccia e vuota o bloccata).
...
- **macOS/Linux** (installazioni vecchie): `~/.local/share/opencode/zyraxon.jsonc`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.config\opencode\zyraxon.jsonc`
...
ZYRAXON puo anche caricare plugin locali dal disco. Spostali temporaneamente altrove (o rinomina la cartella) e riavvia l'app desktop:
...
Se disabilitare i plugin non aiuta (o l'installazione di un plugin e bloccata), svuota la cache in modo che ZYRAXON possa ricostruirla.

1. Chiudi completamente ZYRAXON Desktop.
...
3. Riavvia ZYRAXON Desktop.

---

### Risolvi problemi di connessione al server

ZYRAXON Desktop puo avviare il proprio server locale (predefinito) oppure connettersi a un URL server che hai configurato.
...
Se il tuo `zyraxon.json(c)` contiene una sezione `server`, rimuovila temporaneamente e riavvia l'app desktop.
...
Su Windows, ZYRAXON Desktop richiede **WebView2 Runtime** di Microsoft Edge. Se l'app si apre su una finestra vuota o non parte, installa/aggiorna WebView2 e riprova.

---

### Windows: problemi generali di prestazioni

Se riscontri prestazioni lente, problemi di accesso ai file o problemi del terminale su Windows, prova a usare [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL fornisce un ambiente Linux che funziona in modo piu fluido con le funzionalita di ZYRAXON.

---

### Notifiche non visualizzate

ZYRAXON Desktop mostra le notifiche di sistema solo quando:

- le notifiche sono abilitate per ZYRAXON nelle impostazioni del sistema operativo, e
...
1. Chiudi ZYRAXON Desktop.
2. Trova ed elimina questi file (si trovano nella directory dati dell'app ZYRAXON Desktop):
...
Se riscontri problemi con ZYRAXON:
...
Per capire a quali modelli hai accesso, esegui `zyraxon models`
