...
Puoi aggiungere strumenti esterni a ZYRAXON usando il _Model Context Protocol_, o MCP. ZYRAXON supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [ZYRAXON Config](https://zyraxon.ai/docs/config/) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
ZYRAXON gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, ZYRAXON:
...
Se il server richiede autenticazione, ZYRAXON ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `zyraxon mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, ZYRAXON memorizzera' i token in modo sicuro in `~/.local/share/opencode/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in ZYRAXON insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione ZYRAXON come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare ZYRAXON al tuo account Sentry.
