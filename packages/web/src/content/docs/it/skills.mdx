...
Le skill degli agenti permettono a ZYRAXON di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
ZYRAXON cerca in queste posizioni:
...
Per i percorsi locali al progetto, ZYRAXON risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
...
ZYRAXON elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `zyraxon.json`:
...
**Per agenti integrati** (in `zyraxon.json`):
