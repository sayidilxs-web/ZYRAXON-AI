---
title: Web
description: Usa ZYRAXON nel browser.
---

ZYRAXON puo funzionare come applicazione web nel browser, offrendo la stessa potente esperienza di coding AI senza bisogno di un terminale.

![ZYRAXON Web - Nuova sessione](../../../assets/web/web-homepage-new-session.png)
...
Questo avvia un server locale su `127.0.0.1` con una porta disponibile scelta a caso e apre automaticamente ZYRAXON nel browser predefinito.
...
:::tip[Utenti Windows]
Per la migliore esperienza, esegui `zyraxon web` da [WSL](/docs/windows-wsl) invece che da PowerShell. Questo garantisce accesso corretto al file system e integrazione col terminale.
:::
...
Per impostazione predefinita, ZYRAXON sceglie una porta disponibile. Puoi specificare una porta:
...
Per impostazione predefinita, il server si lega a `127.0.0.1` (solo localhost). Per rendere ZYRAXON accessibile sulla rete:

```bash
opencode web --hostname 0.0.0.0
```

Usando `0.0.0.0`, ZYRAXON mostrera sia l'indirizzo locale sia quello di rete:
...
Una volta avviata, l'interfaccia web ti da accesso alle tue sessioni ZYRAXON.

### Sessioni

Visualizza e gestisci le sessioni dalla homepage. Puoi vedere le sessioni attive e avviarne di nuove.

![ZYRAXON Web - Sessione attiva](../../../assets/web/web-homepage-active-session.png)
...
Puoi anche configurare le impostazioni del server nel file di config `zyraxon.json`:
