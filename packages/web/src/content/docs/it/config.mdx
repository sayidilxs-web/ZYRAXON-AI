...
description: Usa la configurazione JSON di ZYRAXON.
---

Puoi configurare ZYRAXON usando un file di configurazione JSON.

---

## Formato

ZYRAXON supporta sia **JSON** sia **JSONC** (JSON con commenti).
...
2. **Config globale** (`~/.config/opencode/zyraxon.json`) - preferenze utente
3. **Config personalizzata** (variabile d'ambiente `OPENCODE_CONFIG`) - sovrascritture personalizzate
4. **Config di progetto** (`zyraxon.json` nel progetto) - impostazioni specifiche del progetto
...
Metti la configurazione globale di ZYRAXON in `~/.config/opencode/zyraxon.json`. Usa la configurazione globale per preferenze server/runtime valide per l'utente come provider, modelli e permessi.
...
Aggiungi `zyraxon.json` nella root del progetto. La configurazione di progetto ha la precedenza piu alta tra i file standard: sovrascrive sia la configurazione globale sia quella remota.
...
Quando ZYRAXON si avvia, cerca un file di configurazione nella directory corrente oppure risale fino alla directory Git piu vicina.
...
Il file di configurazione ha uno schema definito in [**`zyraxon.ai/config.json`**](https://zyraxon.ai/config.json).

La configurazione TUI usa [**`zyraxon.ai/tui.json`**](https://zyraxon.ai/tui.json).
...
Le chiavi legacy `theme`, `keybinds` e `tui` in `zyraxon.json` sono deprecate e vengono migrate automaticamente quando possibile.
...
Puoi configurare le impostazioni del server per i comandi `zyraxon serve` e `zyraxon web` tramite l'opzione `server`.
...
- `mdns` - Abilita la scoperta del servizio mDNS. Questo permette ad altri dispositivi sulla rete di scoprire il tuo server ZYRAXON.
...
Puoi configurare provider e modelli da usare in ZYRAXON tramite le opzioni `provider`, `model` e `small_model`.
...
L'opzione `small_model` configura un modello separato per task leggeri come la generazione dei titoli. Per impostazione predefinita, ZYRAXON prova a usare un modello più economico se disponibile dal tuo provider, altrimenti fa fallback sul tuo modello principale.
...
L'agente predefinito deve essere un agente primary (non un subagent). Puo essere un agente integrato come `"build"` o `"plan"`, oppure un [agente personalizzato](/docs/agents) che hai definito. Se l'agente specificato non esiste o e un subagent, ZYRAXON fara fallback su `"build"` mostrando un avviso.

Questa impostazione si applica a tutte le interfacce: TUI, CLI (`zyraxon run`), app desktop e GitHub Action.
...
ZYRAXON scarichera automaticamente eventuali aggiornamenti quando si avvia. Puoi disabilitare questa funzione con l'opzione `autoupdate`.
...
[I plugin](/docs/plugins) estendono ZYRAXON con strumenti personalizzati, hook e integrazioni.
...
E utile quando vuoi limitare ZYRAXON a usare solo alcuni provider invece di disabilitarli uno per uno.
