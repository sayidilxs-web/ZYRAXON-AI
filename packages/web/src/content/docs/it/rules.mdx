...
Per gli utenti che migrano da Claude Code, ZYRAXON supporta come fallback le convenzioni di file di Claude Code:
...
Puoi specificare file di istruzioni personalizzati nel tuo `zyraxon.json` o nel globale `~/.config/opencode/zyraxon.json`. Questo permette a te e al tuo team di riusare regole esistenti invece di doverle duplicare in AGENTS.md.
...
L'approccio consigliato e' usare il campo `instructions` in `zyraxon.json`:
...
:::tip
Per monorepo o progetti con standard condivisi, usare `zyraxon.json` con pattern glob (come `packages/*/AGENTS.md`) e' piu' manutenibile rispetto alle istruzioni manuali.
:::
