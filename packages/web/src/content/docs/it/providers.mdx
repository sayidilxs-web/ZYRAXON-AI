---
title: Provider
description: Usa qualsiasi provider LLM in ZYRAXON.
---

import config from "../../../../config.mjs"
export const console = config.console

ZYRAXON usa [AI SDK](https://ai-sdk.dev/) e [Models.dev](https://models.dev) per supportare **oltre 75 provider LLM** e anche modelli locali.

Per aggiungere un provider devi:

1. Aggiungere le API key del provider con il comando `/connect`.
2. Configurare il provider nella configurazione di ZYRAXON.
...
ZYRAXON Zen e una lista di modelli forniti dal team ZYRAXON, testati e
verificati per funzionare bene con ZYRAXON. [Scopri di piu](/docs/zen).

:::tip
Se sei all'inizio, ti consigliamo di partire da ZYRAXON Zen.
:::

1. Esegui `/connect` nella TUI, seleziona opencode e vai su [zyraxon.ai/auth](https://zyraxon.ai/auth).
...
Funziona come qualsiasi altro provider in ZYRAXON ed e totalmente opzionale.
...
Per usare Amazon Bedrock con ZYRAXON:
...
2. **Configura l'autenticazione** usando uno dei seguenti metodi:
   ...
   ***
   ...
   Per configurazione specifica del progetto o persistente, usa `zyraxon.json`:
...
:::info
L'uso dell'abbonamento Claude Pro/Max in ZYRAXON non è ufficialmente supportato da [Anthropic](https://anthropic.com).
:::
...
ZYRAXON usa un modello piccolo per alcuni task AI come generare il titolo della sessione.
È configurato per usare gpt-5-nano di default, ospitato da Zen. Per bloccare ZYRAXON
a usare solo la tua istanza GitLab self-hosted, aggiungi quanto segue al tuo
file `zyraxon.json`. Si raccomanda anche di disabilitare la condivisione sessioni.
...
Personalizza tramite `zyraxon.json`:
...
Per usare Google Vertex AI con ZYRAXON:
...
La funzionalità [Sessions](https://docs.helicone.ai/features/sessions) di Helicone ti permette di raggruppare richieste LLM correlate. Usa il plugin [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) per loggare automaticamente ogni conversazione di ZYRAXON come una sessione in Helicone.
...
Il plugin inietta gli header `Helicone-Session-Id` e `Helicone-Session-Name` nelle tue richieste. Nella pagina Sessions di Helicone, vedrai ogni conversazione di ZYRAXON elencata come sessione separata.
...
:::tip
Ollama può auto-configurarsi per ZYRAXON. Vedi la [documentazione di integrazione Ollama](https://docs.ollama.com/integrations/opencode) per dettagli.
:::
...
Per usare Ollama Cloud con ZYRAXON:
...
3. Copia la chiave API da usare in ZYRAXON.
...
6. **Importante**: Prima di usare modelli cloud in ZYRAXON, devi scaricare le informazioni del modello localmente:
...
ZYRAXON Zen è una lista di modelli testati e verificati forniti dal team ZYRAXON. [Scopri di più](/docs/zen).

1. Accedi a **<a href={console}>ZYRAXON Zen</a>** e clicca **Create API Key**.

2. Esegui il comando `/connect` e cerca **ZYRAXON Zen**.

   ```txt
   /connect
   ```

3. Inserisci la tua chiave API di ZYRAXON.
...
4. Crea o aggiorna il tuo file `zyraxon.json` nella directory del progetto:
...
I campi `limit` permettono a ZYRAXON di capire quanto contesto rimane. I provider standard recuperano questi dati da models.dev automaticamente.
...
1. **Controlla il setup auth**: Esegui `zyraxon auth list` per vedere se le credenziali
