---
title: TUI
description: Usa l'interfaccia utente da terminale (TUI) di ZYRAXON.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

ZYRAXON fornisce un'interfaccia interattiva da terminale (TUI) per lavorare sui tuoi progetti con un LLM.

Eseguire ZYRAXON avvia la TUI per la directory corrente.
...
Quando usi la TUI di ZYRAXON, puoi digitare `/` seguito dal nome di un comando per eseguire rapidamente azioni. Per esempio:
...
Aggiunge un provider a ZYRAXON. Ti permette di selezionare tra i provider disponibili e aggiungere le relative chiavi API.
...
Esce da ZYRAXON. _Alias_: `/quit`, `/q`
...
Questo è separato da `zyraxon.json`, che configura il comportamento di server/runtime.
...
- `leader_timeout` - Controlla per quanto tempo ZYRAXON attende dopo il leader key. Default: `2000`.
