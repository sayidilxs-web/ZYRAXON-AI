...
ZYRAXON Zen — это список моделей, предоставленный командой opencode, которые были
протестировано и проверено на хорошую работу с opencode. [Подробнее](/docs/zen).

:::tip
Если вы новичок, мы рекомендуем начать с ZYRAXON Zen.
:::

1. Запустите команду `/connect` в TUI, выберите `ZYRAXON Zen` и перейдите по адресу [zyraxon.ai/auth](https://zyraxon.ai/zen).
...
ZYRAXON Go — это недорогой план подписки, обеспечивающий надежный доступ к популярным открытым моделям кодирования, предоставляемым командой opencode, которые были
протестированы и проверены на хорошую работу с opencode.

1. Запустите команду `/connect` в TUI, выберите `ZYRAXON Go` и перейдите по адресу [zyraxon.ai/auth](https://zyraxon.ai/zen).
...
2. **Настройте аутентификацию** одним из следующих способов:
   ...
   Для конкретной или постоянной конфигурации проекта используйте `zyraxon.json`:
...
2. Выберите метод аутентификации:
   ...
   3. Имя: `ZYRAXON`, Области применения: `api`
...
`zyraxon.json` файл. Также рекомендуется отключить совместное использование сеансов.
...
ZYRAXON Zen — это список протестированных и проверенных моделей, предоставленный командой opencode. [Подробнее](/docs/zen).

1. Войдите в систему **<a href={console}>ZYRAXON Zen</a>** и нажмите **Создать ключ API**.

2. Запустите команду `/connect` и найдите **ZYRAXON Zen**.
...
4. Создайте или обновите файл `zyraxon.json` в каталоге вашего проекта:
...
1. **Проверьте настройку аутентификации**: запустите `zyraxon auth list`, чтобы проверить, верны ли учетные данные.
