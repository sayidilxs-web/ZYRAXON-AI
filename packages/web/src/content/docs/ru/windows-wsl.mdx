...
1. **Запускайте `zyraxon web` в терминале WSL**, а не в PowerShell:
...
Запуск `zyraxon web` из WSL обеспечивает корректный доступ к файловой системе и интеграцию с терминалом, при этом интерфейс остается доступным из браузера Windows.
