...
2. **Глобальная конфигурация** (`~/.config/opencode/zyraxon.json`) — настройки пользователя.
3. **Пользовательская конфигурация** (`OPENCODE_CONFIG` env var) – пользовательские переопределения
4. **Конфигурация проекта** (`zyraxon.json` в проекте) — настройки, специфичные для проекта.
...
Поместите глобальную конфигурацию opencode в `~/.config/opencode/zyraxon.json`. Используйте глобальную конфигурацию для общепользовательских настроек, таких как темы, поставщики или привязки клавиш.
...
Добавьте `zyraxon.json` в корень вашего проекта. Конфигурация проекта имеет наивысший приоритет среди стандартных файлов конфигурации — она переопределяет как глобальные, так и удаленные конфигурации.
...
Файл конфигурации имеет схему, определенную в [**`zyraxon.ai/config.json`**](https://zyraxon.ai/config.json).

Конфигурация TUI использует [**`zyraxon.ai/tui.json`**](https://zyraxon.ai/tui.json).
...
Устаревшие ключи `theme`, `keybinds` и `tui` в `zyraxon.json` устарели и автоматически переносятся, когда это возможно.
...
Вы можете настроить параметры сервера для команд `zyraxon serve` и `zyraxon web` с помощью опции `server`.
...
Этот параметр применяется ко всем интерфейсам: TUI, CLI (`zyraxon run`), настольному приложению и действию GitHub.
