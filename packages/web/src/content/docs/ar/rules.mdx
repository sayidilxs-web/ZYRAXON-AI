...
للمستخدمين الذين ينتقلون من Claude Code، يدعم ZYRAXON اصطلاحات ملفات Claude Code كبدائل احتياطية:
...
يمكنك تحديد ملفات تعليمات مخصصة في `zyraxon.json` أو في الملف العام `~/.config/opencode/zyraxon.json`. يتيح ذلك لك ولفريقك إعادة استخدام القواعد الموجودة بدلًا من تكرارها داخل AGENTS.md.
...
### استخدام zyraxon.json

النهج الموصى به هو استخدام الحقل `instructions` في `zyraxon.json`:
...
:::tip
بالنسبة للمستودعات متعددة الحزم (monorepos) أو المشاريع ذات المعايير المشتركة، فإن استخدام `zyraxon.json` مع أنماط glob (مثل `packages/*/AGENTS.md`) أسهل للصيانة من التعليمات اليدوية.
:::
