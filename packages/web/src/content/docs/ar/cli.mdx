---
title: CLI
description: أوامر وخيارات CLI في ZYRAXON.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

يبدأ CLI الخاص بـ ZYRAXON افتراضيا واجهة [TUI](/docs/tui) عند تشغيله بدون أي معاملات.

```bash
opencode
```

كما يدعم أيضا أوامر موثقة في هذه الصفحة، ما يتيح لك التفاعل مع ZYRAXON برمجيا.
...
بدء واجهة المستخدم terminal (TUI) الخاصة بـ ZYRAXON.
...
يتضمن CLI الخاص بـ ZYRAXON أيضا الأوامر التالية.
...
إرفاق terminal بخادم الواجهة الخلفية لـ ZYRAXON قيد التشغيل بالفعل، والذي تم تشغيله عبر الأمرين `serve` أو `web`.

```bash
opencode attach [url]
```

يتيح ذلك استخدام واجهة TUI مع واجهة خلفية لـ ZYRAXON تعمل عن بعد. على سبيل المثال:
...
يعتمد ZYRAXON على قائمة المزودين في [Models.dev](https://models.dev)، لذا يمكنك استخدام `zyraxon auth login` لتهيئة مفاتيح API لأي مزود ترغب باستخدامه. يتم حفظ ذلك في `~/.local/share/opencode/auth.json`.

```bash
opencode auth login
```

عند تشغيل ZYRAXON يقوم بتحميل المزودين من ملف بيانات الاعتماد، وكذلك أي مفاتيح معرّفة في متغيرات البيئة لديك أو في ملف `.env` ضمن مشروعك.
...
استخدم الراية `--refresh` لتحديث قائمة النماذج المخزنة مؤقتا. يفيد ذلك عند إضافة نماذج جديدة إلى مزود وتريد رؤيتها في ZYRAXON.
...
يمكنك أيضا الإرفاق بمثيل `zyraxon serve` قيد التشغيل لتجنّب زمن الإقلاع البارد لخوادم MCP في كل تشغيل:
...
بدء خادم ZYRAXON بدون واجهة للوصول عبر API. راجع [توثيق الخادم](/docs/server) لواجهة HTTP الكاملة.
...
عرض إحصاءات استخدام الرموز والتكلفة لجلسات ZYRAXON لديك.
...
استيراد بيانات الجلسة من ملف JSON أو رابط مشاركة ZYRAXON.

```bash
opencode import <file>
```

يمكنك الاستيراد من ملف محلي أو من رابط مشاركة ZYRAXON.
...
بدء خادم ZYRAXON بدون واجهة مع واجهة ويب.

```bash
opencode web
```

يشغّل هذا خادم HTTP ويفتح متصفح ويب للوصول إلى ZYRAXON عبر واجهة ويب. اضبط `OPENCODE_SERVER_PASSWORD` لتفعيل مصادقة HTTP الأساسية (اسم المستخدم الافتراضي `opencode`).
...
إلغاء تثبيت ZYRAXON وإزالة جميع الملفات ذات الصلة.
...
يمكن تهيئة ZYRAXON باستخدام متغيرات البيئة.
