---
title: الإعدادات
description: استخدام ملف إعدادات ZYRAXON بصيغة JSON.
---

يمكنك ضبط ZYRAXON باستخدام ملف إعدادات بصيغة JSON.

---

## التنسيق

يدعم ZYRAXON تنسيقي **JSON** و**JSONC** (JSON مع تعليقات).
...
2. **الإعدادات العامة** (`~/.config/opencode/zyraxon.json`) - تفضيلات المستخدم
3. **إعدادات مخصصة** (`OPENCODE_CONFIG` env var) - تجاوزات مخصصة
4. **إعدادات المشروع** (`zyraxon.json` داخل المشروع) - إعدادات خاصة بالمشروع
...
ضع إعدادات ZYRAXON العامة في `~/.config/opencode/zyraxon.json`. استخدم الإعدادات العامة للتفضيلات على مستوى المستخدم مثل السمات، والمزوّدين، أو اختصارات المفاتيح.
...
أضف `zyraxon.json` في جذر مشروعك. تمتلك إعدادات المشروع أعلى أولوية بين ملفات الإعدادات القياسية، إذ تتجاوز الإعدادات العامة والبعيدة معًا.

:::tip
ضع إعدادات المشروع الخاصة في جذر مشروعك.
:::

عند تشغيل ZYRAXON، يبحث عن ملف إعدادات في الدليل الحالي أو يصعد حتى أقرب دليل Git.
...
يحتوي ملف الإعدادات على مخطط مُعرَّف في [**`zyraxon.ai/config.json`**](https://zyraxon.ai/config.json).
...
يمكنك ضبط إعدادات الخادم لأوامر `zyraxon serve` و`zyraxon web` عبر الخيار `server`.
...
- `mdns` - تفعيل اكتشاف الخدمة عبر mDNS. يتيح ذلك للأجهزة الأخرى على الشبكة اكتشاف خادم ZYRAXON.
...
يمكنك ضبط المزوّدين والنماذج التي تريد استخدامها في إعدادات ZYRAXON عبر الخيارات `provider` و`model` و`small_model`.
...
يضبط الخيار `small_model` نموذجًا منفصلًا للمهام الخفيفة مثل توليد العناوين. افتراضيًا يحاول ZYRAXON استخدام نموذج أقل تكلفة إذا كان متاحًا لدى مزوّدك، وإلا فسيعود إلى النموذج الرئيسي.
...
يمكنك ضبط السمة التي تريد استخدامها في إعدادات ZYRAXON عبر الخيار `theme`.
...
يجب أن يكون الوكيل الافتراضي وكيلًا أساسيًا (وليس وكيلًا فرعيًا). يمكن أن يكون وكيلًا مدمجًا مثل `"build"` أو `"plan"`، أو [وكيلًا مخصصًا](/docs/agents) قمت بتعريفه. إذا لم يكن الوكيل المحدد موجودًا أو كان وكيلًا فرعيًا، فسيعود ZYRAXON إلى `"build"` مع تحذير.

ينطبق هذا الإعداد على جميع الواجهات: TUI وCLI (`zyraxon run`) وتطبيق سطح المكتب وGitHub Action.
...
سيقوم ZYRAXON بتنزيل أي تحديثات جديدة تلقائيًا عند بدء التشغيل. يمكنك تعطيل ذلك عبر الخيار `autoupdate`.
...
[الإضافات](/docs/plugins) توسّع ZYRAXON بأدوات وخُطافات وتكاملات مخصصة.
...
هذا مفيد عندما تريد تقييد ZYRAXON لاستخدام مزوّدين محددين بدلًا من تعطيلهم واحدًا تلو الآخر.
