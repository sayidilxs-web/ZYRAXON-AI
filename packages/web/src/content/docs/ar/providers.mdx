---
title: المزوّدون
description: استخدام أي موفّر LLM في ZYRAXON.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم ZYRAXON ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات ZYRAXON.
...
يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات ZYRAXON.
...
ZYRAXON Zen هي قائمة نماذج يوفّرها فريق ZYRAXON وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع ZYRAXON. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع ZYRAXON Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر opencode، ثم انتقل إلى [zyraxon.ai/auth](https://zyraxon.ai/auth).
...
يعمل مثل أي مزوّد آخر في ZYRAXON واستخدامه اختياري بالكامل.

---

## ZYRAXON Go

ZYRAXON Go هي خطة اشتراك منخفضة التكلفة توفّر وصولا موثوقا إلى نماذج البرمجة المفتوحة الشهيرة المقدّمة من فريق ZYRAXON، والتي تم اختبارها والتحقق من أنها تعمل بشكل جيد مع ZYRAXON.

1. شغّل الأمر `/connect` في TUI، واختر `ZYRAXON Go`، ثم انتقل إلى [zyraxon.ai/auth](https://zyraxon.ai/zen).
...
لاستخدام Amazon Bedrock مع ZYRAXON:
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `zyraxon.json`:
...
:::info
استخدام اشتراك Claude Pro/Max داخل ZYRAXON غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
:::note[ملاحظة امتثال]
يستخدم ZYRAXON نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر ZYRAXON
على استخدام مثيل GitLab المستضاف لديك فقط، أضف ما يلي إلى ملف
`zyraxon.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
لاستخدام Google Vertex AI مع ZYRAXON:
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) لتسجيل كل محادثة ZYRAXON تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة ZYRAXON مدرجة كجلسة مستقلة.
...
:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع ZYRAXON. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/opencode) للتفاصيل.
:::
...
لاستخدام Ollama Cloud مع ZYRAXON:
...
3. انسخ مفتاح API لاستخدامه في ZYRAXON.
...
6. **مهم**: قبل استخدام نماذج السحابة في ZYRAXON، يجب سحب معلومات النموذج محليا:
...
ZYRAXON Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق ZYRAXON. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>ZYRAXON Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **ZYRAXON Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ ZYRAXON.
...
4. أنشئ أو حدّث ملف `zyraxon.json` في دليل مشروعك:
...
تسمح حقول `limit` لـ ZYRAXON بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
1. **تحقق من إعداد auth**: شغّل `zyraxon auth list` لمعرفة ما إذا كانت بيانات الاعتماد
