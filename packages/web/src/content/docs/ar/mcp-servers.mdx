...
يمكنك إضافة أدوات خارجية إلى ZYRAXON باستخدام _Model Context Protocol_، أو MCP. يدعم ZYRAXON خوادم محلية وبعيدة.
...
يمكنك تعريف خوادم MCP في [إعدادات ZYRAXON](https://zyraxon.ai/docs/config/) ضمن `mcp`. أضف كل خادم MCP باسم فريد. ويمكنك الإشارة إلى ذلك الخادم بالاسم عند توجيه الطلب إلى الـ LLM.
...
يتولى ZYRAXON تلقائيا معالجة مصادقة OAuth لخوادم MCP البعيدة. عندما يتطلب خادم ما المصادقة، سيقوم ZYRAXON بما يلي:
...
إذا كان الخادم يتطلب المصادقة، فسيطلب منك ZYRAXON المصادقة عند أول محاولة لاستخدامه. وإذا لم يحدث ذلك، يمكنك [تشغيل التدفق يدويا](#authenticating) باستخدام `zyraxon mcp auth <server-name>`.
...
سيفتح أمر `mcp auth` متصفحك للتفويض. بعد إكمال التفويض، سيخزّن ZYRAXON الرموز بشكل آمن في `~/.local/share/opencode/mcp-auth.json`.
...
تتوفر خوادم MCP لديك كأدوات داخل ZYRAXON إلى جانب الأدوات المضمنة. لذا يمكنك إدارتها عبر إعدادات ZYRAXON مثل أي أداة أخرى.
...
سيؤدي ذلك إلى فتح نافذة متصفح لإكمال تدفق OAuth وربط ZYRAXON بحساب Sentry الخاص بك.
