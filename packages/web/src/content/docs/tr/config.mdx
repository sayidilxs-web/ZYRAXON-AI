...
2. **Genel yapılandırma** (`~/.config/opencode/zyraxon.json`) - kullanıcı tercihleri
3. **Özel yapılandırma** (`OPENCODE_CONFIG` env var) - özel geçersiz kılmalar
4. **Proje yapılandırması** (projedeki `zyraxon.json`) - projeye özgü ayarlar
...
Global opencode yapılandırmanızı `~/.config/opencode/zyraxon.json` içine yerleştirin. Temalar, sağlayıcılar veya tuş atamaları gibi kullanıcı çapındaki tercihler için genel yapılandırmayı kullanın.
...
Proje kökünüze `zyraxon.json` ekleyin. Proje yapılandırması, standart yapılandırma dosyaları arasında en yüksek önceliğe sahiptir; hem genel hem de uzak yapılandırmaları geçersiz kılar.
...
Yapılandırma dosyası [**`zyraxon.ai/config.json`**](https://zyraxon.ai/config.json)'da tanımlanan bir şemaya sahiptir.
...
`zyraxon serve` ve `zyraxon web` komutları için sunucu ayarlarını `server` seçeneği aracılığıyla yapılandırabilirsiniz.
...
Bu ayar tüm arayüzler için geçerlidir: TUI, CLI (`zyraxon run`), masaüstü uygulaması ve GitHub Action.
