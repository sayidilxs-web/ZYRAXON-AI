...
Daha ayrintili ayiklama bilgisi icin `--log-level` komut satiri secenegini kullanabilirsiniz. Ornek: `zyraxon --log-level DEBUG`.
...
- **macOS/Linux**: `~/.config/opencode/zyraxon.jsonc` (veya `~/.config/opencode/zyraxon.json`)
- **macOS/Linux** (eski kurulumlar): `~/.local/share/opencode/zyraxon.jsonc`
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\opencode\zyraxon.jsonc` yapistirin
...
`zyraxon.json(c)` dosyanizda `server` bolumu varsa gecici olarak kaldirin ve desktop uygulamasini yeniden baslatin.
...
3. `zyraxon upgrade` ile en guncel surumu kullandiginizdan emin olun
...
Erisiminiz olan modelleri gormek icin `zyraxon models` calistirin.
