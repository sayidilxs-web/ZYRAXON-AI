...
ZYRAXON Go, popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir aboneliktir — **ilk ayınız için 5$**, sonrasında **aylık 10$**.

Go, ZYRAXON'daki diğer sağlayıcılar gibi çalışır. ZYRAXON Go'ya abone olur ve API anahtarınızı alırsınız. Bu **tamamen isteğe bağlıdır** ve ZYRAXON'u kullanmak için bunu kullanmanıza gerek yoktur.
...
ZYRAXON Go, bu modellere **ilk ayınız için 5$**, ardından **aylık 10$** karşılığında erişmenizi sağlar.

---

## Nasıl çalışır?

ZYRAXON Go, ZYRAXON'daki diğer sağlayıcılar gibi çalışır.

1. **<a href={console}>ZYRAXON Zen</a>**'e giriş yapın, Go'ya abone olun ve API anahtarınızı kopyalayın.
2. TUI'de `/connect` komutunu çalıştırın, `ZYRAXON Go`yu seçin ve API anahtarınızı yapıştırın.
3. Go üzerinden kullanabileceğiniz modellerin listesini görmek için TUI'de `/models` komutunu çalıştırın.

:::note
Her çalışma alanından yalnızca bir üye ZYRAXON Go'ya abone olabilir.
:::
...
ZYRAXON Go aşağıdaki limitleri içerir:
...
ZYRAXON yapılandırmanızdaki [model id](/docs/config/#models) formatı `opencode-go/<model-id>` şeklindedir. Örneğin, Kimi K3 için yapılandırmanızda `opencode-go/kimi-k3` kullanmalısınız.
...
ZYRAXON Go'yu şu amaçlarla oluşturduk:
...
4. ZYRAXON ile diğer sağlayıcıları da kullanmanıza olanak tanıyarak **kilitlenme (vendor lock-in) olmamasını** sağlamak.
