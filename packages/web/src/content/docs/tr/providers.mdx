...
ZYRAXON Zen, opencode ekibi tarafından sağlanan modellerin bir listesidir.
opencode ile iyi çalıştığı test edilip doğrulandı. [Daha fazla bilgi](/docs/zen).

:::tip
Yeniyseniz ZYRAXON Zen ile başlamanızı öneririz.
:::

1. TUI'de `/connect` komutunu çalıştırın, opencode'u seçin ve [zyraxon.ai/auth](https://zyraxon.ai/auth)'ye gidin.
...
ZYRAXON Go, opencode ile iyi çalıştığı test edilmiş ve doğrulanmış, opencode ekibi tarafından sağlanan popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir abonelik planıdır.

1. TUI'de `/connect` komutunu çalıştırın, `ZYRAXON Go`'yu seçin ve [zyraxon.ai/auth](https://zyraxon.ai/zen) adresine gidin.
...
2. **Kimlik doğrulamayı yapılandırın**: Aşağıdaki yöntemlerden birini kullanın:
   ...
   Projeye özel veya kalıcı yapılandırma için `zyraxon.json` kullanın:
...
`zyraxon.json` dosyası. Ayrıca oturum paylaşımını devre dışı bırakmanız da önerilir.
...
`zyraxon.json` üzerinden özelleştirin:
...
ZYRAXON Zen, opencode ekibinin test edip doğruladığı modellerin bir listesidir. [Daha fazla bilgi](/docs/zen).

1. **<a href={console}>ZYRAXON Zen</a>**'de oturum açın ve **API Anahtarı Oluştur**'u tıklayın.

2. `/connect` komutunu çalıştırın ve **ZYRAXON Zen**'i arayın.
...
4. Proje dizininizde `zyraxon.json` dosyanızı oluşturun veya güncelleyin:
...
1. **Kimlik doğrulama kurulumunu kontrol edin**: Kimlik bilgilerinin geçerli olup olmadığını görmek için `zyraxon auth list` komutunu çalıştırın.
