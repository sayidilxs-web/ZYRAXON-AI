...
Özel talimat dosyalarını `zyraxon.json` veya global `~/.config/opencode/zyraxon.json`'de belirtebilirsiniz. Bu, sizin ve ekibinizin, mevcut kuralları AGENTS.md'ye kopyalamak yerine yeniden kullanmasına olanak tanır.
...
### zyraxon.json'u kullanma

Önerilen yaklaşım, `instructions`'deki `zyraxon.json` alanını kullanmaktır:
...
:::tip
Monorepo'lar veya ortak standartlara sahip projeler için, `zyraxon.json`'yi glob desenleriyle (`packages/*/AGENTS.md` gibi) kullanmak, manuel talimatlara göre daha sürdürülebilirdir.
:::
