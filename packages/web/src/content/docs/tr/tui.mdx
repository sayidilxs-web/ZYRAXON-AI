---
title: TUI
description: ZYRAXON terminal kullanıcı arayüzünü kullanma.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

ZYRAXON, projelerinizde LLM ile çalışmak için etkileşimli bir terminal arayüzü (TUI) sunar.

ZYRAXON'u çalıştırdığınızda mevcut dizin için TUI başlar.
...
ZYRAXON TUI kullanırken `/` ve ardından komut adını yazarak hızlı eylemler çalıştırabilirsiniz. Örneğin:
...
ZYRAXON'a bir sağlayıcı ekler. Mevcut sağlayıcıları seçip API anahtarları eklemenizi sağlar.
...
ZYRAXON'dan çıkar. _Takma adlar_: `/quit`, `/q`
...
Bu, sunucu/çalışma zamanı davranışını yapılandıran `zyraxon.json` dosyasından ayrıdır.
...
- `leader_timeout` - ZYRAXON'un leader key sonrasında ne kadar bekleyeceğini kontrol eder. Varsayılan değer `2000`.
