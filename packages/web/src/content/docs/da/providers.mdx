---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i ZYRAXON.
---

import config from "../../../../config.mjs"
export const console = config.console

ZYRAXON bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i ZYRAXON-konfigurationen.
...
Du kan tilpasse udbyderne gennem `provider`-delen i ZYRAXON-konfigurationen.
...
ZYRAXON Zen er en liste over modeller leveret af ZYRAXON-teamet, som er blevet testet og verificeret til at fungere godt med ZYRAXON. [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med ZYRAXON Zen.
:::

1. Kør kommandoen `/connect` i TUI, vælg opencode og gå til [zyraxon.ai/auth](https://zyraxon.ai/auth).
...
Det fungerer som alle andre udbydere i ZYRAXON og er helt valgfrit at bruge.

---

## ZYRAXON Go

ZYRAXON Go er en billig abonnementsplan, der giver pålidelig adgang til populære åbne kodningsmodeller leveret af ZYRAXON-teamet, som er testet og verificeret til at fungere godt med ZYRAXON.

1. Kør kommandoen `/connect` i TUI, vælg `ZYRAXON Go`, og gå til [zyraxon.ai/auth](https://zyraxon.ai/zen).
...
Sådan bruger du Amazon Bedrock med ZYRAXON:
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   For projektspecifik eller vedvarende konfiguration, brug `zyraxon.json`:
...
:::info
At bruge dit Claude Pro/Max-abonnement i ZYRAXON understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
ZYRAXON bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse ZYRAXON
til kun at bruge din egen GitLab-hostede instans, tilføj følgende i din
`zyraxon.json` fil. Det anbefales også at deaktivere sessionsdeling.
...
Sådan bruger du GitHub Copilot-abonnementet med ZYRAXON:
...
Sådan bruger du Google Vertex AI med ZYRAXON:
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) plugin for automatisk at logge hver ZYRAXON-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver ZYRAXON-samtale opført som en separat session.
...
:::tip
Ollama kan automatisk konfigurere sig selv for ZYRAXON. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/opencode) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med ZYRAXON:
...
3. Kopier API-nøglen til brug i ZYRAXON.
...
6. **Vigtigt**: Før du bruger skymodeller i ZYRAXON, skal du hente modelinformationen lokalt:
...
ZYRAXON Zen er en liste over testede og verificerede modeller leveret af ZYRAXON-teamet. [Læs mere](/docs/zen).

1. Log på **<a href={console}>ZYRAXON Zen</a>** og klik på **Create API Key**.

2. Kør kommandoen `/connect` og søg efter **ZYRAXON Zen**.

   ```txt
   /connect
   ```

3. Indtast ZYRAXON API-nøglen.
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med ZYRAXON:
...
4. Opret eller opdater `zyraxon.json`-filen i projektkataloget:
...
`limit`-felterne lader ZYRAXON forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.
...
1. **Tjek godkendelsesopsætningen**: Kør `zyraxon auth list` for at se om legitimationsoplysningerne
