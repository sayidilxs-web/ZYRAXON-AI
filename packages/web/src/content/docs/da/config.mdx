---
title: Konfiguration
description: Ved at bruge ZYRAXON JSON konfig.
---

Du kan konfigurere ZYRAXON ved hjælp af en JSON-konfigurationsfil.

---

## Format

ZYRAXON understøtter både **JSON** og **JSONC** (JSON med kommentarer) formater.
...
2. **Global config** (`~/.config/opencode/zyraxon.json`) - brugerpræferencer
3. **Tilpasset konfiguration** (`OPENCODE_CONFIG` env var) - tilpassede tilsidesættelser
4. **Project config** (`zyraxon.json` i projekt) - projektspecifikke indstillinger
...
Placer din globale ZYRAXON-konfiguration i `~/.config/opencode/zyraxon.json`. Brug global konfiguration til brugerdækkende præferencer som temaer, udbydere eller nøglebindinger.
...
Tilføj `zyraxon.json` i dit projektrod. Project config har den højeste forrang blandt standard config-filer - den tilsidesætter både globale og eksterne config.

:::tip
Placer projektspecifik konfiguration i roden af ​​dit projekt.
:::

Når ZYRAXON starter op, søger den efter en konfigurationsfil i den aktuelle mappe eller går op til den nærmeste Git-mappe.
...
Konfigurationsfilen har et skema, der defineres i [**`zyraxon.ai/config.json`**](https://zyraxon.ai/config.json).
...
Du kan konfigurere serverindstillinger for kommandoerne `zyraxon serve` og `zyraxon web` gennem indstillingen `server`.
...
- `mdns` - Aktiver mDNS-tjenesteopdagelse. Dette gør det muligt for andre enheder på netværket at opdage din ZYRAXON-server.
...
Du kan konfigurere de udbydere og modeller, du vil bruge i din ZYRAXON-konfiguration, gennem mulighederne `provider`, `model` og `small_model`.
...
Indstillingen `small_model` konfigurerer en separat model til lette opgaver som titelgenerering. Som standard forsøger ZYRAXON at bruge en billigere model, hvis en er tilgængelig fra din udbyder, ellers falder den tilbage til din hovedmodel.
...
Du kan konfigurere det tema, du vil bruge i din ZYRAXON-konfiguration, gennem indstillingen `theme`.
...
Standardagenten skal være en primær agent (ikke en subagent). Dette kan være en indbygget agent som `"build"` eller `"plan"`, eller en [custom agent](/docs/agents), du har defineret. Hvis den angivne agent ikke eksisterer eller er en underagent, vil ZYRAXON falde tilbage til `"build"` med en advarsel.

Denne indstilling gælder på tværs af alle grænseflader: TUI, CLI (`zyraxon run`), desktop-app og GitHub Action.
...
ZYRAXON vil automatisk downloade alle nye opdateringer, når den starter op. Du kan deaktivere dette med indstillingen `autoupdate`.
...
[Plugins](/docs/plugins) udvide ZYRAXON med brugerdefinerede værktøjer, kroge og integrationer.
...
Dette er nyttigt, når du vil begrænse ZYRAXON til kun at bruge specifikke udbydere i stedet for at deaktivere dem én efter én.
