...
Du kan gi tilpassede instruksjoner for at ZYRAXON ved at oprete en `AGENTS.md`-fil. Dette ligner på Cursors regler. Den inneholder instruksjoner som vil bli inkludert i LLMs kontekst for at tilpasse oppførselen til ditt spesifikke prosjekt.
...
For brugere som migrerer fra Claude Code, støtter ZYRAXON Claude Codes filkonvensjoner som fallbacks:
...
Du kan spesifisere egendefinerte instruksjonsfiler i din `zyraxon.json` eller den globale `~/.config/opencode/zyraxon.json`. Dette lar deg og teamet ditt gjenbruge eksisterende regler i stedet for at måtte duplisere dem til AGENTS.md.
...
### Brug af zyraxon.json

Den anbefalte tilnærmingen er at bruge feltet `instructions` i `zyraxon.json`:
...
:::tip
For monorepos eller prosjekter med delte standarder er brug av `zyraxon.json` med glob-mønstre (som `packages/*/AGENTS.md`) mer vedlikeholdbart enn manuelle instruksjoner.
:::
