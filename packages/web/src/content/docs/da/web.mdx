---
title: Web
description: Brug af ZYRAXON i din browser.
---

ZYRAXON kan køre som en webapplikation i din browser, hvilket giver den samme kraftfulde AI-kodningsoplevelse uden behov for en terminal.

![ZYRAXON Web - Ny session](../../../assets/web/web-homepage-new-session.png)
...
Dette starter en lokal server på `127.0.0.1` med en tilfældig tilgængelig port og åbner automatisk ZYRAXON i din standardbrowser.
...
:::tip[Windows-brugere]
For den bedste oplevelse skal du køre `zyraxon web` fra [WSL](/docs/windows-wsl) i stedet for PowerShell. Dette sikrer korrekt filsystemadgang og terminalintegration.
:::
...
Som standard vælger ZYRAXON en tilgængelig port. Du kan angive en port:
...
Som standard binder serveren til `127.0.0.1` (kun lokal vært). Sådan gør du ZYRAXON tilgængelig på dit netværk:

```bash
opencode web --hostname 0.0.0.0
```

Når du bruger `0.0.0.0`, vil ZYRAXON vise både lokale adresser og netværksadresser:
...
Når den er startet, giver webgrænsefladen adgang til dine ZYRAXON-sessioner.

### Sessioner

Se og administrer dine sessioner fra hjemmesiden. Du kan se aktive sessioner og starte nye.

![ZYRAXON Web - Active Session](../../../assets/web/web-homepage-active-session.png)

### Serverstatus

Klik på "Se servere" for at se tilsluttede servere og deres status.

![ZYRAXON Web - Se servere](../../../assets/web/web-homepage-see-servers.png)
...
Du kan også konfigurere serverindstillinger i din `zyraxon.json`-konfigurationsfil:
