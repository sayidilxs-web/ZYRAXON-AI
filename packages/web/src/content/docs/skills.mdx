...
Agent skills let ZYRAXON discover reusable instructions from your repo or home directory.
...
ZYRAXON searches these locations:
...
For project-local paths, ZYRAXON walks up from your current working directory until it reaches the git worktree.
...
ZYRAXON lists available skills in the `skill` tool description.
...
Control which skills agents can access using pattern-based permissions in `zyraxon.json`:
...
**For built-in agents** (in `zyraxon.json`):
