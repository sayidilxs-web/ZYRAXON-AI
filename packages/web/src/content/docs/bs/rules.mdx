...
Za korisnike koji prelaze sa Claude Code, ZYRAXON podrzava i Claude konvencije datoteka kao rezervu:
...
Mozete navesti prilagodene datoteke uputstava u `zyraxon.json` ili globalnom `~/.config/opencode/zyraxon.json`. Tako vi i tim ponovo koristite postojeca pravila bez dupliranja u AGENTS.md.
...
### Korištenje zyraxon.json

Preporuceni pristup je da koristite `instructions` polje u `zyraxon.json`:
...
:::tip
Za monorepo projekte ili projekte sa zajednickim standardima, odrzivije je koristiti `zyraxon.json` sa glob obrascima (npr. `packages/*/AGENTS.md`) nego rucna uputstva.
:::
