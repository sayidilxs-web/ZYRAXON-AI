---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u ZYRAXON.
---

import config from "../../../../config.mjs"
export const console = config.console

ZYRAXON koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj ZYRAXON konfiguraciji.
...
Možete prilagoditi dobavljače putem odjeljka `provider` u vašem ZYRAXON
config.
...
ZYRAXON Zen je lista modela koje je obezbedio ZYRAXON tim koji su bili
testirano i potvrđeno da dobro radi sa ZYRAXON. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa ZYRAXON Zen.
:::
...
Radi kao i svaki drugi provajder u ZYRAXON i potpuno je opcionalan za korištenje.

---

## ZYRAXON Go

ZYRAXON Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža ZYRAXON tim i koji su testirani i verificirani da dobro rade s ZYRAXON-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `ZYRAXON Go` i idite na [zyraxon.ai/auth](https://zyraxon.ai/zen).
...
Da biste koristili Amazon Bedrock s ZYRAXON:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `zyraxon.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u ZYRAXON.
:::
...
ZYRAXON koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate ZYRAXON
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`zyraxon.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Prilagodite putem `zyraxon.json`:
...
Za korištenje Google Vertex AI s ZYRAXON:
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) da automatski prijavite svaki ZYRAXON razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki ZYRAXON razgovor naveden kao zasebna sesija.
...
:::tip
Ollama se može automatski konfigurirati za ZYRAXON. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/opencode) za detalje.
:::
...
Da biste koristili Ollama Cloud s ZYRAXON:
...
3. Kopirajte API ključ za korištenje u ZYRAXON.
...
6. **Važno**: Prije upotrebe modela oblaka u ZYRAXON, morate lokalno povući informacije o modelu:
...
ZYRAXON Zen je lista testiranih i verifikovanih modela koju je obezbedio ZYRAXON tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>ZYRAXON Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **ZYRAXON Zen**.

```txt
   /connect
```

3. Unesite svoj ZYRAXON API ključ.
...
4. Kreirajte ili ažurirajte svoju `zyraxon.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju ZYRAXON da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `zyraxon auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite ZYRAXON konfiguraciju i:
