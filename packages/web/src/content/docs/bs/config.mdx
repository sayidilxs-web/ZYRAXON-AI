---
title: Konfiguracija
description: Korištenje ZYRAXON JSON konfiguracije.
---

Možete konfigurirati ZYRAXON koristeći JSON konfiguracijski fajl.

---

## Format

ZYRAXON podržava i **JSON** i **JSONC** (JSON sa komentarima) formate.
...
2. **Globalna konfiguracija** (`~/.config/opencode/zyraxon.json`) - korisničke preferencije
3. **Prilagođena konfiguracija** (`OPENCODE_CONFIG` env var) - prilagođena preinačenja
4. **Konfiguracija projekta** (`zyraxon.json` u projektu) - postavke specifične za projekat
...
Postavite svoju globalnu ZYRAXON konfiguraciju u `~/.config/opencode/zyraxon.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.
...
Dodajte `zyraxon.json` u korijen projekta. Konfiguracija projekta ima najveći prioritet među standardnim konfiguracijskim datotekama - ona nadjačava globalne i udaljene konfiguracije.
...
Kada se ZYRAXON pokrene, traži konfiguracijsku datoteku u trenutnom direktoriju ili prelazi do najbližeg Git direktorija.
...
Konfiguracijski fajl ima šemu koja je definirana u [**`zyraxon.ai/config.json`**](https://zyraxon.ai/config.json).

TUI konfiguracija koristi [**`zyraxon.ai/tui.json`**](https://zyraxon.ai/tui.json).
...
Stari `theme`, `keybinds`, i `tui` ključevi u `zyraxon.json` su zastarjeli i automatski će se migrirati kada je to moguće.
...
Možete konfigurirati postavke servera za naredbe `zyraxon serve` i `zyraxon web` putem opcije `server`.
...
- `mdns` - Omogući mDNS otkrivanje servisa. Ovo omogućava drugim uređajima na mreži da otkriju vaš ZYRAXON server.
...
Možete konfigurirati dobavljače i modele koje želite koristiti u svojoj ZYRAXON konfiguraciji kroz opcije `provider`, `model` i `small_model`.
...
Opcija `small_model` konfigurira poseban model za lagane zadatke poput generiranja naslova. Podrazumevano, ZYRAXON pokušava da koristi jeftiniji model ako je dostupan od vašeg provajdera, inače se vraća na vaš glavni model.
...
Zadani agent mora biti primarni agent (ne podagent). Ovo može biti ugrađeni agent kao što je `"build"` ili `"plan"`, ili [prilagođeni agent](/docs/agents) koji ste definirali. Ako navedeni agent ne postoji ili je podagent, ZYRAXON će se vratiti na `"build"` s upozorenjem.

Ova postavka se primjenjuje na sva sučelja: TUI, CLI (`zyraxon run`), desktop aplikaciju i GitHub Action.
...
ZYRAXON će automatski preuzeti sva nova ažuriranja kada se pokrene. Ovo možete onemogućiti opcijom `autoupdate`.
...
Prema zadanim postavkama, ZYRAXON **dopušta sve operacije** bez potrebe za eksplicitnim dopuštenjem. Ovo možete promijeniti koristeći opciju `permission`.
...
[Plugins](/docs/plugins) proširuju ZYRAXON sa prilagođenim alatima, kukicama i integracijama.
...
Ovo je korisno kada želite da ograničite ZYRAXON da koristi samo određene provajdere umjesto da ih onemogućavate jednog po jednog.
