...
Možete dodati vanjske alate u ZYRAXON koristeći _Model Context Protocol_, ili MCP. ZYRAXON podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [ZYRAXON Config](https://zyraxon.ai/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
ZYRAXON automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, ZYRAXON će:
...
Ako server zahtijeva autentifikaciju, ZYRAXON će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `zyraxon mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, ZYRAXON sigurno čuva tokene u `~/.local/share/opencode/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u ZYRAXON, zajedno s ugrađenim alatima. Možete njima upravljati kroz ZYRAXON konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete ZYRAXON sa vašim Sentry nalogom.
