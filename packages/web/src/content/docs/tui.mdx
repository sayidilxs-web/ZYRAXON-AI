---
title: TUI
description: Using the ZYRAXON terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

ZYRAXON provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running ZYRAXON starts the TUI for the current directory.
...
When using the ZYRAXON TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to ZYRAXON. Allows you to select from available providers and add their API keys.
...
This is separate from `zyraxon.json`, which configures server/runtime behavior.
...
- `leader_timeout` - Controls how long ZYRAXON waits after the leader key. Defaults to `2000`.
