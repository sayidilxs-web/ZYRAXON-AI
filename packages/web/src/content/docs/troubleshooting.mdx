...
To debug issues with ZYRAXON, start by checking the logs and local data it stores on disk.
...
You can set the log level with the `--log-level` command-line option to get more detailed debug information. For example, `zyraxon --log-level DEBUG`.
...
To uninstall the ZYRAXON CLI and remove its related files, run:
...
To remove ZYRAXON Desktop, uninstall the application through your operating system's app management tools.

---

## Desktop app

ZYRAXON Desktop runs a local ZYRAXON server (the `opencode-cli` sidecar) in the background. Most issues are caused by a misbehaving plugin, a corrupted cache, or a bad server setting.

### Quick checks

- Fully quit and relaunch the app.
- If the app shows an error screen, click **Restart** and copy the error details.
- macOS only: `ZYRAXON` menu -> **Reload Webview** (helps if the UI is blank/frozen).
...
- **macOS/Linux**: `~/.config/opencode/zyraxon.jsonc` (or `~/.config/opencode/zyraxon.json`)
- **macOS/Linux** (older installs): `~/.local/share/opencode/zyraxon.jsonc`
- **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.config\opencode\zyraxon.jsonc`
...
ZYRAXON can also load local plugins from disk. Temporarily move these out of the way (or rename the folder) and restart the desktop app:
...
If disabling plugins doesn't help (or a plugin install is stuck), clear the cache so ZYRAXON can rebuild it.

1. Quit ZYRAXON Desktop completely.
...
3. Restart ZYRAXON Desktop.

---

### Fix server connection issues

ZYRAXON Desktop can either start its own local server (default) or connect to a server URL you configured.
...
If your `zyraxon.json(c)` contains a `server` section, temporarily remove it and restart the desktop app.
...
On Windows, ZYRAXON Desktop requires the Microsoft Edge **WebView2 Runtime**. If the app opens to a blank window or won't start, install/update WebView2 and try again.

---

### Windows: General performance issues

If you're experiencing slow performance, file access issues, or terminal problems on Windows, try using [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL provides a Linux environment that works more seamlessly with ZYRAXON's features.

---

### Notifications not showing

ZYRAXON Desktop only shows system notifications when:

- notifications are enabled for ZYRAXON in your OS settings, and
...
1. Quit ZYRAXON Desktop.
2. Find and delete these files (they live in the ZYRAXON Desktop app data directory):
...
If you're experiencing issues with ZYRAXON:
...
3. Ensure you have the latest version with `zyraxon upgrade`
...
To figure out what models you have access to, run `zyraxon models`
