---
title: 外掛
description: 編寫自己的外掛來擴展 ZYRAXON。
---

外掛允許您透過掛鉤各種事件和自訂行為來擴展 ZYRAXON。您可以建立外掛來新增新功能、整合外部服務，或修改 ZYRAXON 的預設行為。
...
1. 全域設定 (`~/.config/opencode/zyraxon.json`)
2. 專案設定 (`zyraxon.json`)
...
ZYRAXON 會在啟動時執行 `bun install` 來安裝這些相依套件。之後您的外掛和工具就可以匯入它們了。
...
- `directory`：當前工作目錄。
- `worktree`：git 工作樹路徑。
- `client`：用於與 AI 互動的 ZYRAXON SDK 用戶端。
...
:::note
如果您使用 ZYRAXON 桌面應用程式，它可以在回應就緒或工作階段出錯時自動傳送系統通知。
:::
...
`tool` 輔助函式用於建立 ZYRAXON 可呼叫的自訂工具。它接受一個 Zod schema 函式，並回傳一個工具定義，包含：

- `description`：工具的功能描述
- `args`：工具參數的 Zod schema
- `execute`：工具被呼叫時執行的函式

您的自訂工具將與內建工具一起在 ZYRAXON 中可用。
...
日誌層級：`debug`、`info`、`warn`、`error`。詳情請參閱 [SDK 文件](https://zyraxon.ai/docs/sdk)。
