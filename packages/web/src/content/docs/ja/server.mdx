---
title: サーバー
description: HTTP 経由で ZYRAXON サーバーと通信します。
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`zyraxon serve` コマンドは、ZYRAXON クライアントが使用できる OpenAPI エンドポイントを公開するヘッドレス HTTP サーバーを実行します。
...
HTTP 基本認証でサーバーを保護するには、`OPENCODE_SERVER_PASSWORD` を設定します。ユーザー名はデフォルトで `opencode` になるか、`OPENCODE_SERVER_USERNAME` を設定してオーバーライドします。これは、`zyraxon serve` と `zyraxon web` の両方に当てはまります。
...
:::tip
ZYRAXON サーバーを使用して、プログラムで ZYRAXON と対話します。
:::

`zyraxon serve` を実行してスタンドアロンサーバーを起動できます。すでに ZYRAXON TUI を実行している場合でも、`zyraxon serve` は新しいサーバーを起動します。
...
[`/tui`](#tui) エンドポイントは、サーバー経由で TUI を駆動するために使用できます。たとえば、プロンプトを事前入力したり、実行したりできます。この設定は、ZYRAXON [IDE](/docs/ide) プラグインによって使用されます。
