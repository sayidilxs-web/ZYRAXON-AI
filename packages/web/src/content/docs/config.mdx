---
title: Config
description: Using the ZYRAXON JSON config.
---

You can configure ZYRAXON using a JSON config file.

---

## Format

ZYRAXON supports both **JSON** and **JSONC** (JSON with Comments) formats.

```jsonc title="zyraxon.jsonc"
{
...
2. **Global config** (`~/.config/opencode/zyraxon.json`) - user preferences
3. **Custom config** (`OPENCODE_CONFIG` env var) - custom overrides
4. **Project config** (`zyraxon.json` in project) - project-specific settings
...
Place your global ZYRAXON config in `~/.config/opencode/zyraxon.json`. Use global config for user-wide server/runtime preferences like providers, models, and permissions.
...
Add `zyraxon.json` in your project root. Project config has the highest precedence among standard config files - it overrides both global and remote configs.
...
When ZYRAXON starts up, it first looks for a config file in the current directory, then traverses up to the nearest Git directory.
...
Drop an `zyraxon.json` or `zyraxon.jsonc` file in the system managed config directory:
...
On macOS, ZYRAXON reads managed preferences from the `ai.opencode.managed` preference domain. Deploy a `.mobileconfig` via MDM (Jamf, Kandji, FleetDM) and the settings are enforced automatically.

ZYRAXON checks these paths:

1. `/Library/Managed Preferences/<user>/ai.opencode.managed.plist`
2. `/Library/Managed Preferences/ai.opencode.managed.plist`

The plist keys map directly to `zyraxon.json` fields. MDM metadata keys (`PayloadUUID`, `PayloadType`, etc.) are stripped automatically.

**Creating a `.mobileconfig`**

Use the `ai.opencode.managed` PayloadType. The ZYRAXON config keys go directly in the payload dict:
...
The server/runtime config schema is defined in [**`zyraxon.ai/config.json`**](https://zyraxon.ai/config.json).

TUI config uses [**`zyraxon.ai/tui.json`**](https://zyraxon.ai/tui.json).
...
Legacy `theme`, `keybinds`, and `tui` keys in `zyraxon.json` are deprecated and automatically migrated when possible.

---

### Server

You can configure server settings for the `zyraxon serve` and `zyraxon web` commands through the `server` option.
...
- `mdns` - Enable mDNS service discovery. This allows other devices on the network to discover your ZYRAXON server.
...
If not specified, ZYRAXON will automatically discover and use a sensible default based on your operating system (e.g. `pwsh` or `cmd.exe` on Windows, `/bin/zsh` or `/bin/bash` on macOS/Linux). You can provide an absolute path or a short name.
...
You can configure the providers and models you want to use in your ZYRAXON config through the `provider`, `model` and `small_model` options.
...
The `small_model` option configures a separate model for lightweight tasks like title generation. By default, ZYRAXON tries to use a cheaper model if one is available from your provider, otherwise it falls back to your main model.
...
Use the `experimental.policies` option to allow or deny ZYRAXON actions on configured resources. Currently, policies can control which providers ZYRAXON may use.
...
ZYRAXON normalizes image attachments before sending them to the model. By default, images are resized when they exceed `2000x2000` pixels or `5242880` base64 bytes.
...
If an image still cannot fit after resizing, ZYRAXON omits oversized tool-result images or fails oversized user-provided images with an image size error.
...
The default agent must be a primary agent (not a subagent). This can be a built-in agent like `"build"` or `"plan"`, or a [custom agent](/docs/agents) you've defined. If the specified agent doesn't exist or is a subagent, ZYRAXON will fall back to `"build"` with a warning.

This setting applies across all interfaces: TUI, CLI (`zyraxon run`), desktop app, and GitHub Action.
...
ZYRAXON uses snapshots to track file changes during agent operations, enabling you to undo and revert changes within a session. Snapshots are enabled by default.
...
ZYRAXON will automatically download any new updates when it starts up. You can disable this with the `autoupdate` option.
...
[Plugins](/docs/plugins) extend ZYRAXON with custom tools, hooks, and integrations.
...
This is useful when you want to restrict ZYRAXON to only use specific providers rather than disabling them one by one.
