...
Para los usuarios que migran desde Claude Code, ZYRAXON admite las convenciones de archivos de Claude Code como alternativas:
...
Puede especificar archivos de instrucciones personalizados en su `zyraxon.json` o el `~/.config/opencode/zyraxon.json` global. Esto le permite a usted y a su equipo reutilizar las reglas existentes en lugar de tener que duplicarlas en AGENTS.md.
...
El enfoque recomendado es utilizar el campo `instructions` en `zyraxon.json`:
...
:::tip
Para monorepos o proyectos con estándares compartidos, usar `zyraxon.json` con patrones globales (como `packages/*/AGENTS.md`) es más fácil de mantener que las instrucciones manuales.
:::
