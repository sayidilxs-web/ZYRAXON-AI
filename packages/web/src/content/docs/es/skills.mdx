...
Las habilidades del agente permiten a ZYRAXON descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
ZYRAXON busca estas ubicaciones:
...
Para las rutas locales del proyecto, ZYRAXON sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
...
ZYRAXON enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `zyraxon.json`:
...
**Para agentes integrados** (en `zyraxon.json`):
