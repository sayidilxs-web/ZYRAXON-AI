---
title: Complementos
description: Escriba sus propios complementos para extender ZYRAXON.
---

Los complementos le permiten extender ZYRAXON al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de ZYRAXON.
...
1. Configuración global (`~/.config/opencode/zyraxon.json`)
2. Configuración del proyecto (`zyraxon.json`)
...
ZYRAXON ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.
...
:::note
Si está utilizando la aplicación de escritorio ZYRAXON, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Niveles: `debug`, `info`, `warn`, `error`. Consulte la [documentación del SDK](https://zyraxon.ai/docs/sdk) para obtener más detalles.
