---
title: CLI
description: Opciones y comandos de la CLI de ZYRAXON.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

La CLI de ZYRAXON de forma predeterminada inicia el [TUI](/docs/tui) cuando se ejecuta sin ningún argumento.

```bash
opencode
```

Pero también acepta comandos como se documenta en esta página. Esto le permite interactuar con ZYRAXON mediante programación.
...
Inicie la interfaz de usuario del terminal ZYRAXON.
...
La CLI de ZYRAXON también tiene los siguientes comandos.

---

### agent

Administrar agentes para ZYRAXON.
...
Conecte una terminal a un servidor backend ZYRAXON que ya se esté ejecutando y iniciado mediante los comandos `serve` o `web`.

```bash
opencode attach [url]
```

Esto permite usar TUI con un backend remoto ZYRAXON. Por ejemplo:
...
ZYRAXON funciona con la lista de proveedores en [Models.dev](https://models.dev), por lo que puede usar `zyraxon auth login` para configurar las claves API para cualquier proveedor que desee utilizar. Esto se almacena en `~/.local/share/opencode/auth.json`.

```bash
opencode auth login
```

Cuando se inicia ZYRAXON, carga los proveedores desde el archivo de credenciales. Y si hay claves definidas en sus entornos o un archivo `.env` en su proyecto.
...
Utilice el indicador `--refresh` para actualizar la lista de modelos almacenados en caché. Esto es útil cuando se han agregado nuevos modelos a un proveedor y desea verlos en ZYRAXON.
...
También puede conectarse a una instancia `zyraxon serve` en ejecución para evitar tiempos de arranque en frío del servidor MCP en cada ejecución:
...
Inicie un servidor ZYRAXON sin cabeza para acceso API. Consulte los [documentos del servidor](/docs/server) para conocer la interfaz HTTP completa.
...
Enumere todas las sesiones ZYRAXON.
...
Muestre el uso de tokens y las estadísticas de costos para sus sesiones ZYRAXON.
...
Importe datos de sesión desde un archivo JSON o una URL compartida ZYRAXON.

```bash
opencode import <file>
```

Puede importar desde un archivo local o una URL compartida ZYRAXON.
...
Inicie un servidor ZYRAXON sin cabeza con una interfaz web.

```bash
opencode web
```

Esto inicia un servidor HTTP y abre un navegador web para acceder a ZYRAXON a través de una interfaz web. Configure `OPENCODE_SERVER_PASSWORD` para habilitar la autenticación básica HTTP (el nombre de usuario predeterminado es `opencode`).
...
Desinstale ZYRAXON y elimine todos los archivos relacionados.
...
La CLI de ZYRAXON toma las siguientes banderas globales.
...
ZYRAXON se puede configurar mediante variables de entorno.
