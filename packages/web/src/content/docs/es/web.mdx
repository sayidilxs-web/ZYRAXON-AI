---
title: Web
description: Usando ZYRAXON en su navegador.
---

ZYRAXON puede ejecutarse como una aplicación web en su navegador, brindando la misma poderosa experiencia de codificación de IA sin necesidad de una terminal.

![ZYRAXON Web - Nueva sesión](../../../assets/web/web-homepage-new-session.png)
...
Esto inicia un servidor local en `127.0.0.1` con un puerto disponible aleatorio y abre automáticamente ZYRAXON en su navegador predeterminado.
...
Para obtener la mejor experiencia, ejecute `zyraxon web` desde [WSL](/docs/windows-wsl) en lugar de PowerShell. Esto garantiza el acceso adecuado al sistema de archivos y la integración del terminal.
:::
...
De forma predeterminada, ZYRAXON elige un puerto disponible. Puede especificar un puerto:
...
De forma predeterminada, el servidor se vincula a `127.0.0.1` (solo localhost). Para hacer que ZYRAXON sea accesible en su red:

```bash
opencode web --hostname 0.0.0.0
```

Al usar `0.0.0.0`, ZYRAXON mostrará direcciones locales y de red:
...
Una vez iniciada, la interfaz web proporciona acceso a sus sesiones ZYRAXON.

### Sesiones

Vea y administre sus sesiones desde la página de inicio. Puedes ver sesiones activas e iniciar otras nuevas.

![ZYRAXON Web - Sesión activa](../../../assets/web/web-homepage-active-session.png)
...
También puede configurar los ajustes del servidor en su archivo de configuración `zyraxon.json`:
