---
title: Configuración
description: Usando la configuración ZYRAXON JSON.
---

Puede configurar ZYRAXON usando un archivo de configuración JSON.

---

## Formato

ZYRAXON admite los formatos **JSON** y **JSONC** (JSON con comentarios).
...
2. **Configuración global** (`~/.config/opencode/zyraxon.json`) - preferencias del usuario
3. **Configuración personalizada** (`OPENCODE_CONFIG` env var): anulaciones personalizadas
4. **Configuración del proyecto** (`zyraxon.json` en el proyecto): configuración específica del proyecto
...
Coloque su configuración global ZYRAXON en `~/.config/opencode/zyraxon.json`. Utilice la configuración global para las preferencias de todo el usuario, como temas, proveedores o combinaciones de teclas.
...
Agregue `zyraxon.json` en la raíz de su proyecto. La configuración del proyecto tiene la mayor prioridad entre los archivos de configuración estándar: anula las configuraciones globales y remotas.

:::tip
Coloque la configuración específica del proyecto en la raíz de su proyecto.
:::

Cuando se inicia ZYRAXON, busca un archivo de configuración en el directorio actual o recorre hasta el directorio Git más cercano.
...
El archivo de configuración tiene un esquema definido en [**`zyraxon.ai/config.json`**](https://zyraxon.ai/config.json).
...
Puede configurar los ajustes del servidor para los comandos `zyraxon serve` y `zyraxon web` a través de la opción `server`.
...
- `mdns`: habilita el descubrimiento de servicios mDNS. Esto permite que otros dispositivos en la red descubran su servidor ZYRAXON.
...
Puede configurar los proveedores y modelos que desea utilizar en su configuración ZYRAXON a través de las opciones `provider`, `model` y `small_model`.
...
La opción `small_model` configura un modelo separado para tareas livianas como la generación de títulos. De forma predeterminada, ZYRAXON intenta utilizar un modelo más económico si su proveedor tiene uno disponible; de ​​lo contrario, recurre a su modelo principal.
...
Puede configurar el tema que desea usar en su configuración ZYRAXON a través de la opción `theme`.
...
El agente predeterminado debe ser un agente principal (no un subagente). Puede ser un agente integrado como `"build"` o `"plan"`, o un [agente personalizado](/docs/agents) que haya definido. Si el agente especificado no existe o es un subagente, ZYRAXON recurrirá a `"build"` con una advertencia.

Esta configuración se aplica en todas las interfaces: TUI, CLI (`zyraxon run`), aplicación de escritorio y GitHub Acción.
...
ZYRAXON descargará automáticamente cualquier actualización nueva cuando se inicie. Puede desactivar esto con la opción `autoupdate`.
...
Los [complementos](/docs/plugins) amplían ZYRAXON con herramientas, enlaces e integraciones personalizados.
...
Esto es útil cuando desea restringir ZYRAXON para que utilice únicamente proveedores específicos en lugar de deshabilitarlos uno por uno.
