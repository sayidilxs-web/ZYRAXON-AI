...
Puede agregar herramientas externas a ZYRAXON utilizando el _Model Context Protocol_ o MCP. ZYRAXON admite servidores locales y remotos.
...
Puede definir servidores MCP en su [ZYRAXON Config](https://zyraxon.ai/docs/config/) en `mcp`. Agregue cada MCP con un nombre único. Puede referirse a ese MCP por su nombre cuando solicite el LLM.
...
ZYRAXON maneja automáticamente la autenticación OAuth para servidores MCP remotos. Cuando un servidor requiere autenticación, ZYRAXON:
...
Si el servidor requiere autenticación, ZYRAXON le pedirá que se autentique cuando intente usarlo por primera vez. De lo contrario, puede [activar manualmente el flujo](#authenticating) con `zyraxon mcp auth <server-name>`.
...
El comando `mcp auth` abrirá su navegador para obtener autorización. Después de su autorización, ZYRAXON almacenará los tokens de forma segura en `~/.local/share/opencode/mcp-auth.json`.
...
Sus MCP están disponibles como herramientas en ZYRAXON, junto con herramientas integradas. Para que puedas administrarlos a través de la configuración ZYRAXON como cualquier otra herramienta.
...
Esto abrirá una ventana del navegador para completar el flujo OAuth y conectar ZYRAXON a su cuenta Sentry.
