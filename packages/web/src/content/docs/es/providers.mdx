---
title: Proveedores
description: Usando cualquier proveedor LLM en ZYRAXON.
---

import config from "../../../../config.mjs"
export const console = config.console

ZYRAXON usa el [AI SDK](https://ai-sdk.dev/) y [Models.dev](https://models.dev) para admitir **75+ proveedores LLM** y admite la ejecución de modelos locales.

Para agregar un proveedor necesita:

1. Agregue las claves API para el proveedor usando el comando `/connect`.
2. Configure el proveedor en su configuración ZYRAXON.
...
Puedes personalizar los proveedores a través de la sección `provider` en tu ZYRAXON
configuración.
...
ZYRAXON Zen es una lista de modelos proporcionados por el equipo ZYRAXON que han sido
probado y verificado para funcionar bien con ZYRAXON. [Más información](/docs/zen).

:::tip
Si eres nuevo, te recomendamos comenzar con ZYRAXON Zen.
:::

1. Ejecute el comando `/connect` en TUI, seleccione opencode y diríjase a [zyraxon.ai/auth](https://zyraxon.ai/auth).
...
Funciona como cualquier otro proveedor en ZYRAXON y su uso es completamente opcional.

---

## ZYRAXON Go

ZYRAXON Go es un plan de suscripción de bajo costo que brinda acceso confiable a modelos de codificación abiertos populares proporcionados por el equipo de ZYRAXON que han sido
probado y verificado para funcionar bien con ZYRAXON.

1. Ejecute el comando `/connect` en TUI, seleccione `ZYRAXON Go` y diríjase a [zyraxon.ai/auth](https://zyraxon.ai/zen).
...
Para usar Amazon Bedrock con ZYRAXON:
...
2. **Configure la autenticación** utilizando uno de los siguientes métodos:
   ...
   Para una configuración persistente o específica del proyecto, utilice `zyraxon.json`:
...
:::info
El uso de su suscripción Claude Pro/Max en ZYRAXON no está oficialmente respaldado por [Anthropic](https://anthropic.com).
:::
...
ZYRAXON utiliza un modelo pequeño para algunas tareas de IA, como generar el título de la sesión.
Está configurado para usar gpt-5-nano de forma predeterminada, alojado por Zen. Para bloquear ZYRAXON
Para usar solo su propia instancia alojada en GitLab, agregue lo siguiente a su
Archivo `zyraxon.json`. También se recomienda desactivar el uso compartido de sesiones.
...
Personalizar a través de `zyraxon.json`:
...
Para utilizar Google Vertex AI con ZYRAXON:
...
La función [Sesiones](https://docs.helicone.ai/features/sessions) de Helicone le permite agrupar solicitudes LLM relacionadas. Utilice el complemento [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) para registrar automáticamente cada conversación ZYRAXON como una sesión en Helicone.
...
El complemento inyecta encabezados `Helicone-Session-Id` y `Helicone-Session-Name` en sus solicitudes. En la página Sesiones de Helicone, verá cada conversación ZYRAXON enumerada como una sesión separada.
...
:::tip
Ollama puede configurarse automáticamente para ZYRAXON. Consulte los [documentos de integración de Ollama](https://docs.ollama.com/integrations/opencode) para obtener más detalles.
:::
...
Para usar Ollama Cloud con ZYRAXON:
...
3. Copie la clave API para usarla en ZYRAXON.
...
6. **Importante**: Antes de usar modelos en la nube en ZYRAXON, debe obtener la información del modelo localmente:
...
ZYRAXON Zen es una lista de modelos probados y verificados proporcionada por el equipo ZYRAXON. [Más información](/docs/zen).

1. Inicie sesión en **<a href={console}>ZYRAXON Zen</a>** y haga clic en **Crear API clave**.

2. Ejecute el comando `/connect` y busque **ZYRAXON Zen**.

   ```txt
   /connect
   ```

3. Ingrese su clave ZYRAXON API.
...
Para utilizar [API generativas de Scaleway](https://www.scaleway.com/en/docs/generative-apis/) con ZYRAXON:
...
4. Cree o actualice su archivo `zyraxon.json` en el directorio de su proyecto:
...
Los campos `limit` le permiten a ZYRAXON comprender cuánto contexto le queda. Los proveedores estándar los extraen de models.dev automáticamente.
...
1. **Verifique la configuración de autenticación**: Ejecute `zyraxon auth list` para ver si las credenciales
