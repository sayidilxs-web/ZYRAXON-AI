...
ZYRAXON Zen é uma lista de modelos fornecidos pela equipe do opencode que foram testados e verificados para funcionar bem com o opencode. [Saiba mais](/docs/zen).

:::tip
Se você é novo, recomendamos começar com o ZYRAXON Zen.
:::

1. Execute o comando `/connect` no TUI, selecione `ZYRAXON Zen` e acesse [zyraxon.ai/zen](https://zyraxon.ai/zen).
...
ZYRAXON Go é um plano de assinatura de baixo custo que fornece acesso confiável a modelos de codificação abertos populares fornecidos pela equipe do opencode que foram
testados e verificados para funcionar bem com o opencode.

1. Execute o comando `/connect` no TUI, selecione `ZYRAXON Go` e acesse [zyraxon.ai/zen](https://zyraxon.ai/zen).
...
2. **Configure a autenticação** usando um dos seguintes métodos:
   ...
   ***
   ...
   Para configuração específica do projeto ou persistente, use `zyraxon.json`:
...
arquivo `zyraxon.json`. Também é recomendado desativar o compartilhamento de sessões.
...
Personalize através do `zyraxon.json`:
...
ZYRAXON Zen é uma lista de modelos testados e verificados fornecidos pela equipe do opencode. [Saiba mais](/docs/zen).

1. Faça login em **<a href={console}>ZYRAXON Zen</a>** e clique em **Criar Chave da API**.

2. Execute o comando `/connect` e procure por **ZYRAXON Zen**.
...
3. Insira sua chave da API para o provedor.
   ...
   ┌  Adicionar credencial
   │
   ▲  Isso armazena apenas uma credencial para myprovider - você precisará configurá-lo no zyraxon.json, verifique a documentação para exemplos.
   │
...
4. Crie ou atualize seu arquivo `zyraxon.json` no diretório do seu projeto:
...
1. **Verifique a configuração de autenticação**: Execute `zyraxon auth list` para ver se as credenciais
