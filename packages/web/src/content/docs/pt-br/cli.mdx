...
O opencode é alimentado pela lista de provedores em [Models.dev](https://models.dev), então você pode usar `zyraxon auth login` para configurar chaves de API para qualquer provedor que você gostaria de usar. Isso é armazenado em `~/.local/share/opencode/auth.json`.
...
Você também pode se anexar a uma instância em execução do `zyraxon serve` para evitar tempos de inicialização a frio do servidor MCP em cada execução:
