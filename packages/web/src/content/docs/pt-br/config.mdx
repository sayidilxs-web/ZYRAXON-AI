...
2. **Configuração global** (`~/.config/opencode/zyraxon.json`) - preferências do usuário
3. **Configuração personalizada** (`OPENCODE_CONFIG` var de ambiente) - substituições personalizadas
4. **Configuração do projeto** (`zyraxon.json` no projeto) - configurações específicas do projeto
...
Coloque sua configuração global do opencode em `~/.config/opencode/zyraxon.json`. Use a configuração global para preferências de todo o usuário, como provedores, modelos e permissões.
...
Adicione `zyraxon.json` na raiz do seu projeto. A configuração do projeto tem a maior precedência entre os arquivos de configuração padrão - ela substitui tanto as configurações globais quanto as remotas.
...
O esquema de configuração do servidor/tempo de execução é definido em [**`zyraxon.ai/config.json`**](https://zyraxon.ai/config.json).

A configuração do TUI usa [**`zyraxon.ai/tui.json`**](https://zyraxon.ai/tui.json).
...
Chaves legadas `theme`, `keybinds` e `tui` em `zyraxon.json` estão obsoletas e são migradas automaticamente quando possível.
...
Você pode configurar as configurações do servidor para os comandos `zyraxon serve` e `zyraxon web` através da opção `server`.
...
Essa configuração se aplica a todas as interfaces: TUI, CLI (`zyraxon run`), aplicativo desktop e GitHub Action.
