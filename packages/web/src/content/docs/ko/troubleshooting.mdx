...
자세한 디버그 정보를 얻기 위해 `--log-level` 명령줄 옵션을 사용하여 로그 레벨을 설정할 수 있습니다. 예를 들면, `zyraxon --log-level DEBUG`.
...
- macOS만: `ZYRAXON` 메뉴 -> **웹뷰 **(UI가 공백/frozen인 경우).
...
-**macOS/리눅스**: `~/.config/opencode/zyraxon.jsonc` (또는 `~/.config/opencode/zyraxon.json`) -**macOS/Linux** (외부 설치): `~/.local/share/opencode/zyraxon.jsonc`

- **Windows**: 압박 `WIN+R`와 풀 `%USERPROFILE%\.config\opencode\zyraxon.jsonc`
...
`zyraxon.json(c)`가 `server` 섹션을 포함하면 일시적으로 제거하고 데스크톱 앱을 다시 시작합니다.
...
3. 당신은 `zyraxon upgrade`를 가진 최신 버전이 있는 것을 지킵니다
...
당신이 액세스 할 수있는 모델을 파악하려면, `zyraxon models`를 실행
