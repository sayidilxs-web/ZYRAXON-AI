...
Trim `packages/ZYRAXON/src/server/routes/instance/httpapi/public.ts` until OpenAPI generation is mostly a direct projection of the `HttpApi` route declarations, without breaking the generated SDK surface.
...
- `[x]` Add or extend `packages/ZYRAXON/test/server/httpapi-query-schema-drift.test.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/ZYRAXON`.
- `[x]` `bun typecheck` from `packages/ZYRAXON`.

### PR 2: Delete Spec-Only Workspace Query Injection

- `[x]` Edit `packages/ZYRAXON/src/server/routes/instance/httpapi/public.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/ZYRAXON`.
- `[x]` `bun dev generate > /tmp/ZYRAXON-openapi.json` from `packages/ZYRAXON`.
...
- Edit `packages/ZYRAXON/src/server/routes/instance/httpapi/public.ts`.
...
- `bun dev generate > /tmp/ZYRAXON-openapi.json` from `packages/ZYRAXON`.
...
- Check source schemas in files like `packages/ZYRAXON/src/session/schema.ts`, `packages/ZYRAXON/src/permission/schema.ts`, and pty schema definitions.
...
- Edit route group files under `packages/ZYRAXON/src/server/routes/instance/httpapi/groups/`.
- Replace SDK-visible `HttpApiError.BadRequest` / `HttpApiError.NotFound` with explicit error schemas from `packages/ZYRAXON/src/server/routes/instance/httpapi/errors.ts` or add new ones there.
