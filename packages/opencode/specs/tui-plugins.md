...
- Author package entrypoint is `@ZYRAXON-ai/plugin/tui`.
...
```json
{
  "$schema": "https://ZYRAXON.ai/tui.json",
  "theme": "smoke-theme",
  "leader_timeout": 2000,
  "keybinds": {
    "leader": "ctrl+x",
  ...
  "plugin": ["@acme/ZYRAXON-plugin@1.2.3", ["./plugins/demo.tsx", { "label": "demo" }]],
  "plugin_enabled": {
    "acme.demo": false
  },
  "attention": {
...
- Import types from `@ZYRAXON-ai/plugin/tui`.
- `@ZYRAXON-ai/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.
...
- The value is a semver range checked against the running ZYRAXON version.
- If the range is not satisfied, the plugin is skipped with a warning and a session error.
- If `engines.ZYRAXON` is absent, no check is performed (backward compatible).
...
- `ZYRAXON plugin <module>` and TUI install both run install → manifest read → config patch.
- Alias: `ZYRAXON plug <module>`.
- `-g` / `--global` writes into the global config dir.
- Local installs resolve target dir inside `patchPluginConfig`.
- For local scope, path is `<worktree>/.ZYRAXON` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.ZYRAXON`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.ZYRAXON`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.ZYRAXON` dir or `ZYRAXON_CONFIG_DIR`, ZYRAXON installs `@ZYRAXON-ai/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@ZYRAXON-ai/plugin/tui`.
...
- The host already installs the default OpenTUI bundle (`default keys`, metadata fields, and enabled fields) plus ZYRAXON's comma bindings, leader token, base layout fallback, pending-sequence helpers, and managed textarea layer.
...
ZYRAXON registers a `mode` layer field on the host keymap. Plugins can use it to keep bindings active only in the relevant UI state.
...
Mode pushes are automatically tracked by the plugin runtime. If a plugin is disabled, fails during activation, or the TUI shuts down before the plugin calls the disposer, ZYRAXON pops the plugin's pushed modes during plugin cleanup. Calling the disposer yourself is still recommended for component lifetimes; cleanup remains idempotent.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@ZYRAXON-ai/plugin/tui`.
...
- `message` is required; `title` defaults to `"ZYRAXON"`; `notification` defaults to enabled with `when: "blurred"`; `sound` defaults to enabled with `when: "always"`.
...
- Config `attention.sounds` overrides active-pack sounds by slot. Failed loads fall back to the active pack and then `ZYRAXON.default`.
...
- Local plugins persist installed themes under the local `.ZYRAXON/themes` area near the plugin config source.
...
- `--pure` / `ZYRAXON_PURE` skips external TUI plugins only.
...
- Local smoke plugin: `.ZYRAXON/plugins/tui-smoke.tsx`
- Local vim plugin: `.ZYRAXON/plugins/tui-vim.tsx`
- Local smoke config: `.ZYRAXON/tui.json`
- Local smoke theme: `.ZYRAXON/plugins/smoke-theme.json`
