...
Delete `packages/ZYRAXON/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/ZYRAXON/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/ZYRAXON/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/ZYRAXON`
- TUI plugin loader tests in `packages/ZYRAXON` if runtime plugin API wiring changed
