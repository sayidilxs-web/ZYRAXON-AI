...
Practical reference for a future `packages/server` split after the ZYRAXON
server moved to the Effect HttpApi backend.

## Current State

- The server still lives in `packages/ZYRAXON`.
...
Until enough shared service code lives outside `packages/ZYRAXON`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/ZYRAXON`

It should not import `packages/ZYRAXON` services while `packages/ZYRAXON`
...
2. Move stable domain schemas into shared packages only when they no longer
   depend on ZYRAXON-local runtime modules.
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/ZYRAXON` implementation details.
