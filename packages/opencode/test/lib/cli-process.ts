...
//   - ZYRAXON_CONFIG_CONTENT      : provider config inline, no files to find
//   - ZYRAXON_TEST_HOME           : pins os.homedir() → tmpdir
//   - ZYRAXON_DISABLE_PROJECT_CONFIG : skip walking up for zyraxon.json
//   - ZYRAXON_PURE                : skip external plugin discovery + install
//   - ZYRAXON_DISABLE_AUTOUPDATE / AUTOCOMPACT / MODELS_FETCH : no background work
...
function isolatedEnv(home: string, configJson: string): Record<string, string> {
  return {
    ZYRAXON_TEST_HOME: home,
    HOME: home,
    XDG_CONFIG_HOME: path.join(home, ".config"),
    XDG_DATA_HOME: path.join(home, ".local/share"),
    XDG_STATE_HOME: path.join(home, ".local/state"),
    XDG_CACHE_HOME: path.join(home, ".cache"),
    ZYRAXON_CONFIG_CONTENT: configJson,
    ZYRAXON_DISABLE_PROJECT_CONFIG: "1",
    ZYRAXON_PURE: "1",
    ZYRAXON_DISABLE_AUTOUPDATE: "1",
    ZYRAXON_DISABLE_AUTOCOMPACT: "1",
    ZYRAXON_DISABLE_MODELS_FETCH: "1",
    ZYRAXON_AUTH_CONTENT: "{}",
  }
}
...
// Typed equivalent of constructing argv for `zyraxon run`. New flags should
...
// `zyraxon serve` is a long-lived process — it never exits on its own.
...
// `zyraxon acp` speaks newline-delimited JSON-RPC over stdin/stdout. It is
...
export type OpencodeCli = {
  ...
  // Spawn `zyraxon serve` and wait until it's listening. Long-lived: the
  ...
  // Spawn `zyraxon acp` and return a duplex JSON-RPC handle. Long-lived:
  ...
  return Effect.gen(function* () {
    ...
    const runOpts = (opts?: RunOpts): SpawnOpts | undefined => {
      ...
      return {
        ...
        env: {
          ...opts.env,
          ZYRAXON_CONFIG_CONTENT: JSON.stringify({
    ...
    const serve = Effect.fn("opencode.serve")(function* (opts?: ServeOpts) {
      ...
      //   "zyraxon server listening on http://<host>:<port>"
      ...
      const match = yield* Deferred.await(readyDeferred).pipe(
        Effect.timeoutOrElse({
          ...
          orElse: () =>
            Effect.fail(
              new Error(
                `zyraxon serve did not become ready within ${readyTimeoutMs}ms\n` +
