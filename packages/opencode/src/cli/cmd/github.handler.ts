...
jobs:
  opencode:
    ...
    steps:
      ...
      - name: Run opencode
        uses: onelpawarai/ZYRAXON-AI/github@latest${envStr}
        with:
...
export const githubRun = Effect.fn("Cli.github.run")(function* (args: { event?: string; token?: string }) {
  ...
  yield* Effect.promise(async () => {
    ...
    const shareBaseUrl = isMock ? "https://dev.zyraxonai.lovable.app" : "https://zyraxonai.lovable.app"
    ...
    function normalizeOidcBaseUrl(): string {
      ...
      if (!value) return "https://api.zyraxon.ai"
