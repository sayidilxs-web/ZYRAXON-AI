...
// CLI entry point for `zyraxon run` and `zyraxon --mini`.
//
// Handles three modes:
//   1. Non-interactive (default): sends a single prompt, streams events to
//      stdout, and exits when the session goes idle.
//   2. Interactive local (`zyraxon --mini`): boots the split-footer direct mode
//      with an in-process server (no external HTTP).
//   3. Interactive attach (`zyraxon --mini --attach`): connects to a running
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to ZYRAXON_SERVER_PASSWORD)",
      })
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: "basic auth username (defaults to ZYRAXON_SERVER_USERNAME or 'opencode')",
      })
      .option("dir", {
        type: "string",
