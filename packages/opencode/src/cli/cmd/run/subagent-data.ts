...
function taskTab(part: ToolPart, sessionID: string): FooterSubagentTab {
  const rawType = text(part.state.input.subagent_type)
  const label = rawType
    ? Locale.titlecase(rawType)
    : Locale.titlecase(text(metadata(part, "agentName")) ?? text(metadata(part, "agent")) ?? "general")
