...
export const McpAuthCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.mcp.auth")(function* (args) {
    ...
    if (servers.length === 0) {
      ...
      prompts.log.info("Remote MCP servers support OAuth by default. Add a remote server in zyraxon.json:")
      prompts.log.info(`
  "mcp": {
    "my-server": {
      "type": "remote",
...
async function resolveConfigPath(baseDir: string, global = false) {
  // Check for existing config files (prefer .jsonc over .json, check .zyraxon/ subdirectory too)
  const candidates = [path.join(baseDir, "zyraxon.json"), path.join(baseDir, "zyraxon.jsonc")]

  if (!global) {
    candidates.push(path.join(baseDir, ".zyraxon", "zyraxon.json"), path.join(baseDir, ".zyraxon", "zyraxon.jsonc"))
  }
  ...
  // Default to zyraxon.json if none exist
  return candidates[0]
}
