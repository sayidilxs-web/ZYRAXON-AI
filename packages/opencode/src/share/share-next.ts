...
const disabled = process.env["ZYRAXON_DISABLE_SHARE"] === "true" || process.env["ZYRAXON_DISABLE_SHARE"] === "1"
...
export class Service extends Context.Service<Service, Interface>()("@zyraxon/ShareNext") {}
...
const publicApi = api("public/shares")
  ...
  Effect.gen(function* () {
    ...
    const state: InstanceState.InstanceState<State> = yield* InstanceState.make<State>(
      Effect.fn("ShareNext.state")(function* (_ctx) {
        ...
        yield* watch(Session.Event.Diff, (data) =>
          sync(data.sessionID, [{ type: "diff", data: structuredClone(data.diff) as SDK.SnapshotFileDiff[] }]),
        )
    ...
    const request = Effect.fn("ShareNext.request")(function* () {
      ...
      if (Option.isNone(active) || !active.value.active_org_id) {
        const baseUrl = (yield* cfg.get()).enterprise?.url ?? "https://zyraxonai.lovable.app"
        return { headers, api: publicApi, baseUrl } satisfies Req
      }
