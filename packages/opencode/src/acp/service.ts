...
export class Service extends Context.Service<Service, Interface>()("@zyraxon/ACP/Service") {}
...
export function make(input: {
  ...
  const initialize = Effect.fn("ACP.initialize")(function* (params: InitializeRequest) {
    ...
    const authMethod: AuthMethod = {
      description: "Run `zyraxon auth login` in the terminal",
...
function defaultModelFromConfig(
  ...
  const opencodeProvider = providers[ProviderV2.ID.make("zyraxon")]
...
function fromUnknownError(error: unknown, service?: string): Error {
  ...
  return new ACPError.ServiceFailureError({ safeMessage: "ZYRAXON service failure", service })
}
