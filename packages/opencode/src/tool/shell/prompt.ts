...
function shellDisplayName(name: string) {
  ...
  if (name === "elvish") return "Elvish"
  if (name === "nu") return "Nushell"
  if (name === "xonsh") return "Xonsh"
...
function fishNotes() {
  return `# Fish shell notes
- Fish uses \`set\` for variables instead of \`export\`. Use \`set -x VAR value\` to export.
- Use \`source\` or \`.\` to source files.
- Fish does not support \`&&\` or \`;\` for chaining. Use \`; and\` or \`; or\` instead.
- Use \`string escape\` for escaping strings.
- Use \`abbr\` or \`alias\` for command shortcuts.`
}

function nushellNotes() {
  return `# Nushell notes
- Nushell uses structured data (tables, lists) instead of plain text.
- Use \`let\` for variable assignment: \`let x = 1\`
- Use \`mut\` for mutable variables: \`mut x = 1\`
- Pipeline operators \`|\` work as expected.
- Use \`each\`, \`where\`, \`select\` for data manipulation.
- Commands: \`open\` (read file), \`save\` (write file), \`ls\` (list), \`cd\` (change dir).`
}
...
function chainGuidance(name: string) {
  ...
  if (FISH.has(name)) {
    return "Fish does not support '&&'. Use '; and' to chain dependent commands (e.g., `git add . ; and git commit -m \"message\" ; and git push`). For parallel independent commands, make multiple tool calls in one message."
  }
  if (NUSHELL.has(name)) {
    return "Nushell supports pipeline operators. Use ';' to chain dependent commands (e.g., `git add .; git commit -m \"message\"; git push`). For parallel independent commands, make multiple tool calls in one message."
  }
...
'@`,
  ...
  if (FISH.has(name)) {
    return {
      ...
      gitCommands: "fish commands",
      gitCommandRestriction: "fish commands",
  ...
  if (NUSHELL.has(name)) {
    return {
      ...
      workdirSection:
        "All commands run in the current working directory by default. Use the `workdir` parameter if you need to run a command in a different directory. AVOID using `cd <directory> && <command>` patterns - use `workdir` instead.",
      commandSection: bashCommandSection(chain, limits, defaultTimeoutMs),
      gitCommands: "nushell commands",
      gitCommandRestriction: "nushell commands",
      createPrInstruction: "Create PR using gh pr create with the format below. Use a string to pass the body correctly.",
