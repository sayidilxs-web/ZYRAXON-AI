...
export class Service extends Context.Service<Service, Interface>()("@zyraxon/Config") {}

export const use = serviceUse(Service)

function globalConfigFile() {
  const candidates = ["zyraxon.jsonc", "zyraxon.json", "config.json"].map((file) =>
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadGlobal = Effect.fnUntraced(function* (env?: Record<string, string>) {
      ...
      if (!Flag.ZYRAXON_CONFIG && !Flag.ZYRAXON_CONFIG_DIR && !Flag.ZYRAXON_CONFIG_CONTENT) {
      ...
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "zyraxon.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "zyraxon.jsonc"), env))
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        const pluginScopeForSource = Effect.fnUntraced(function* (source: string) {
          ...
          if (source === "ZYRAXON_CONFIG_CONTENT") return "local"
        ...
        if (Flag.ZYRAXON_CONFIG) {
          yield* merge(Flag.ZYRAXON_CONFIG, yield* loadFile(Flag.ZYRAXON_CONFIG, authEnv))
          yield* Effect.logDebug("loaded custom config", { path: Flag.ZYRAXON_CONFIG })
        }

        if (!Flag.ZYRAXON_DISABLE_PROJECT_CONFIG) {
        ...
        if (Flag.ZYRAXON_CONFIG_DIR) {
          yield* Effect.logDebug("loading config from ZYRAXON_CONFIG_DIR", { path: Flag.ZYRAXON_CONFIG_DIR })
        }

        const deps: Fiber.Fiber<void>[] = []

        for (const dir of directories) {
          if (dir.endsWith(".zyraxon") || dir === Flag.ZYRAXON_CONFIG_DIR) {
            for (const file of ["zyraxon.json", "zyraxon.jsonc"]) {
          ...
          // Auto-discovered plugins under `.zyraxon/plugin(s)` are already local files, so ConfigPlugin.load
          // returns normalized Specs and we only need to attach origin metadata here.
          const list = yield* Effect.promise(() => ConfigPlugin.load(dir))
          yield* mergePluginOrigins(dir, list)
        }

        if (process.env.ZYRAXON_CONFIG_CONTENT) {
          const source = "ZYRAXON_CONFIG_CONTENT"
          const next = yield* loadConfig(process.env.ZYRAXON_CONFIG_CONTENT, {
            dir: ctx.directory,
            source,
          })
          yield* merge(source, next, "local")
          yield* Effect.logDebug("loaded custom config from ZYRAXON_CONFIG_CONTENT")
        }
        ...
        if (activeAccount?.active_org_id) {
          ...
          yield* Effect.gen(function* () {
            ...
            if (Option.isSome(tokenOpt)) {
              process.env["ZYRAXON_CONSOLE_TOKEN"] = tokenOpt.value
              yield* env.set("ZYRAXON_CONSOLE_TOKEN", tokenOpt.value)
            }
        ...
        if (Flag.ZYRAXON_PERMISSION) {
          try {
            result.permission = mergeDeep(result.permission ?? {}, JSON.parse(Flag.ZYRAXON_PERMISSION))
          } catch (err) {
            yield* Effect.logWarning("ZYRAXON_PERMISSION contains invalid JSON, skipping", { err })
          }
        }
        ...
        if (Flag.ZYRAXON_DISABLE_AUTOCOMPACT) {
          result.compaction = { ...result.compaction, auto: false }
        }
        if (Flag.ZYRAXON_DISABLE_PRUNE) {
