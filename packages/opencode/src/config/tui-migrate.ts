...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from zyraxon.json files
...
async function opencodeFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...
    ...(await Filesystem.findUp(["zyraxon.json", "zyraxon.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "opencode"))
  }
  if (Flag.ZYRAXON_CONFIG) files.push(Flag.ZYRAXON_CONFIG)
