...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".zyraxon"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".zyraxon", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".zyraxon", "tui.json"),
  }
}
...
async function load(input: {
  ...
  try {
    ...
    const records = Flag.ZYRAXON_PURE ? [] : pluginOrigins
    if (Flag.ZYRAXON_PURE && pluginOrigins.length) {
    }
